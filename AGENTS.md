# GNFS Project — Codex Instructions
...
GNFS 提供 62 个 `GNFS_*` 运行时调优开关 / helper(SIMD 内核、并行 dispatcher、cache / pool、实验通道)。**详细设计**(算法、ENV 解析、bit-for-bit 保证、ROI、集成点、测试)按模块收录在 [`docs/env-flags/`](docs/env-flags/README.md);下表为一句话速查。

> **共性约定**:`auto|0|1` 是三态 SIMD gate(`auto` = 平台 / 尺寸可用即启用);并行 dispatcher 用整数 `N`,default `1`(sequential,零开销);cache / pool 用整数 `N`,default `0`(disabled)。几乎所有开关保证开 / 关 **bit-for-bit 一致**(仅性能不同),由单元测试强制。许多 helper 当前主 pipeline **未 wire-in**,是预留基础设施。共性详见 [docs/env-flags/README.md](docs/env-flags/README.md#通用设计约定)。

### 关系处理 (relation) — [完整文档](docs/env-flags/relation.md)

| ENV | 默认 | 用途 |
|-----|------|------|
| [`GNFS_CASCADE_V3`](docs/env-flags/relation.md#filter-merge--v0--v3-clique-cascade) | OFF | V0+V3 clique cascade merge(weight≥3 LP key) |
| [`GNFS_OVERRIDE_LP_BITS`](docs/env-flags/relation.md#lp_bits-实验-gnfs_override_lp_bits) | digit 默认 | runtime 覆盖 digit-based lp_bits |
| [`GNFS_V0_WEIGHT3`](docs/env-flags/relation.md#v0-weight-3-merge-gnfs_v0_weight3) | 0 | V0 也 merge weight=3 LP key 前 2 partials |
| [`GNFS_DROP_RESIDUAL` / `GNFS_WEIGHT_CUTOFF`](docs/env-flags/relation.md#drop-residual--weight-cutoff-50d-β-plateau-实验通道) | 0 | drop 残留 LP partials / weight 截断(50d β plateau 实验) |
| [`GNFS_V0_BFS`](docs/env-flags/relation.md#v0-bfs-chain-merge-gnfs_v0_bfs) | 0 | V0 主路径用 BFS spanning tree 合并 |
| [`GNFS_OOC_RELATIONS`](docs/env-flags/relation.md#ooc-relation-store-gnfs_ooc_relations) | 0 | sieve 期间 relations 流式写盘降内存 |
| [`GNFS_RELATION_POOL_SIZE`](docs/env-flags/relation.md#relation-collector-memory-pool-gnfs_relation_pool_size) | 0 | RelationCollector pmr 内存池 |
| [`GNFS_FILTER_RADIX_SORT`](docs/env-flags/relation.md#phase-0-radix-sort-dedup-sort-gnfs_filter_radix_sort) | 0 | Phase 0 dedup-sort 用 LSD radix |
| [`GNFS_FILTER_LP_BLOOM_BITS`](docs/env-flags/relation.md#filter-phase-0-lp-key-bloom-pre-screen-gnfs_filter_lp_bloom_bits) | 0 | LP key 去重 Bloom pre-screen |
| [`GNFS_FILTER_LP_HASH_MIX`](docs/env-flags/relation.md#lp-key-splitmix64-hash-mixing-gnfs_filter_lp_hash_mix) | auto | LP key splitmix64 hash 混合 |
| [`GNFS_FILTER_MERGE_THREADS`](docs/env-flags/relation.md#partial-relation-merger-并行-gnfs_filter_merge_threads) | 1 | LP-key bucket merge 并行 |

### 线性代数 (linalg) — [完整文档](docs/env-flags/linalg.md)

| ENV | 默认 | 用途 |
|-----|------|------|
| [`GNFS_NO_THIN_SOLVE`](docs/env-flags/linalg.md#thin-matrix-bw-solve-bmtm-variant-default-on) | thin ON | opt-out thin matrix BW solve(B′=MᵀM) |
| [`GNFS_BW_KRYLOV_MMAP`](docs/env-flags/linalg.md#bw-krylov-sequence-mmap-gnfs_bw_krylov_mmap) | 0 | BW Phase 1 Krylov 序列 mmap 到磁盘 |
| [`GNFS_BW_KRYLOV_STREAMS`](docs/env-flags/linalg.md#bw-krylov-multi-stream-parallel-gnfs_bw_krylov_streams) | 1 | BW Krylov multi-stream 并发 retry |
| [`GNFS_SPMV_SIMD`](docs/env-flags/linalg.md#simd-gf2-spmv-inner-kernels-gnfs_spmv_simd) | auto | GF(2) SpMV 内核 NEON/AVX2 宽 XOR |
| [`GNFS_SCHIROKAUER_THREADS`](docs/env-flags/linalg.md#schirokauer-map-每关系并行-gnfs_schirokauer_threads) | 1 | Schirokauer map 每关系并行 |
| [`GNFS_SGE_BATCH_PIVOTS`](docs/env-flags/linalg.md#sge-batch-pivot-选择-gnfs_sge_batch_pivots) | 1 | SGE 每 pass 批量选 disjoint pivot |
| [`GNFS_MATRIX_TRANSPOSE_BLOCKED`](docs/env-flags/linalg.md#cache-blocked-gf2-matrix-transpose-gnfs_matrix_transpose_blocked) | auto | 64×64 tile 块化 GF(2) 转置 |
| [`GNFS_GF2_POPCNT_SIMD`](docs/env-flags/linalg.md#gf2-word-popcount-simd-batch-gnfs_gf2_popcnt_simd) | auto | GF(2) word 批量 popcount SIMD |
| [`GNFS_GF2_AND_POPCNT_SIMD`](docs/env-flags/linalg.md#gf2-and-popcount-simd-batch-gnfs_gf2_and_popcnt_simd) | auto | popcount(a&b) 批量 SIMD |
| [`GNFS_GF2_XOR_POPCNT_SIMD`](docs/env-flags/linalg.md#gf2-xor-popcount-simd-batch-gnfs_gf2_xor_popcnt_simd) | auto | popcount(a^b) Hamming 距离 SIMD |
| [`GNFS_GF2_ROW_POPCOUNT_SIMD`](docs/env-flags/linalg.md#gf2-per-row-popcount-simd-batch-gnfs_gf2_row_popcount_simd) | auto | 行主序矩阵 per-row popcount SIMD |
| [`GNFS_GF2_ROW_XOR_SIMD`](docs/env-flags/linalg.md#gf2-row-word-xor-simd-batch-gnfs_gf2_row_xor_simd) | auto | in-place dst^=src 批量 SIMD |
| [`GNFS_GF2_AND_WORDS_SIMD`](docs/env-flags/linalg.md#gf2-word-and-batch-simd-gnfs_gf2_and_words_simd) | auto | out=a&b 批量 SIMD(保留 vector) |
| [`GNFS_LINALG_PROGRESS_INTERVAL`](docs/env-flags/linalg.md#linalg-迭代进度遥测-gnfs_linalg_progress_interval) | 0 | BL/BW 迭代进度遥测 |

### 余因子分解 (cofactor) — [完整文档](docs/env-flags/cofactor.md)

| ENV | 默认 | 用途 |
|-----|------|------|
| [`GNFS_TRIAL_DIV_SIMD`](docs/env-flags/cofactor.md#trial-division-simd-8-prime-batch-gnfs_trial_div_simd) | auto | trial division 8-prime batch SIMD |
| [`GNFS_SURVIVAL_FILTER` / `GNFS_SURVIVAL_THRESHOLD`](docs/env-flags/cofactor.md#cofactor-survival-rate-predictor-gnfs_survival_filter--gnfs_survival_threshold) | OFF | Dickman ρ cofactor survival 早 reject |
| [`GNFS_COFACTOR_TIMING_ENABLE`](docs/env-flags/cofactor.md#cofactor-per-stage-timing-telemetry-gnfs_cofactor_timing_enable) | 0 | cofactor 6-stage 计时遥测 |
| [`GNFS_ECM_B1_CACHE_SIZE`](docs/env-flags/cofactor.md#ecm-b1-prime-power-expansion-cache-gnfs_ecm_b1_cache_size) | 0 | ECM B1 prime-power 展开缓存 |
| [`GNFS_COFACTOR_RESULT_CACHE_SIZE`](docs/env-flags/cofactor.md#cofactor-classification-result-cache-gnfs_cofactor_result_cache_size) | 0 | cofactor 分类结果 LRU cache |
| [`GNFS_ECM_SIGMA_POOL_SIZE`](docs/env-flags/cofactor.md#ecm-sigma-seed-warm-pool-gnfs_ecm_sigma_pool_size) | 0 | ECM sigma seed warm pool |
| [`GNFS_ECM_STAGE2_PARALLEL`](docs/env-flags/cofactor.md#ecm-stage-2-多曲线并行-gnfs_ecm_stage2_parallel) | 1 | ECM Stage 2 多曲线并行 |
| [`GNFS_ECM_STAGE1_PARALLEL_THREADS`](docs/env-flags/cofactor.md#ecm-stage-1-多曲线并行-gnfs_ecm_stage1_parallel_threads) | 1 | ECM Stage 1 多曲线并行 |
| [`GNFS_ECM_BATCH_INV`](docs/env-flags/cofactor.md#ecm-montgomery-batch-inversion-gnfs_ecm_batch_inv) | 0 | ECM Montgomery batch inversion |
| [`GNFS_BRENT_POLLARD_RHO_THREADS`](docs/env-flags/cofactor.md#brent-pollard-rho-多配置并行-gnfs_brent_pollard_rho_threads) | 1 | Brent rho 多配置并行 |

### 多项式 (polynomial) — [完整文档](docs/env-flags/polynomial.md)

| ENV | 默认 | 用途 |
|-----|------|------|
| [`GNFS_MURPHY_ALPHA_THREADS`](docs/env-flags/polynomial.md#murphy-e-alpha-并行-gnfs_murphy_alpha_threads) | hw | Murphy E compute_alpha 并行 |
| [`GNFS_POLY_HGCD`](docs/env-flags/polynomial.md#polynomial-half-gcd-gnfs_poly_hgcd) | 0 | Half-GCD over F_p[x] |
| [`GNFS_POLY_KARATSUBA_THRESHOLD`](docs/env-flags/polynomial.md#polynomial-karatsuba-multiplication-threshold-gnfs_poly_karatsuba_threshold) | 32 | Karatsuba 乘法 threshold |
| [`GNFS_POLY_NTT`](docs/env-flags/polynomial.md#polynomial-ntt-multiplication-gnfs_poly_ntt) | auto | 3-prime CRT NTT 乘法 |
| [`GNFS_POLY_SQUARE_OPT`](docs/env-flags/polynomial.md#polynomial-modular-squaring-gnfs_poly_square_opt) | auto | 多项式模平方(对角对称省半工) |
| [`GNFS_POLY_ADD_MOD_SIMD`](docs/env-flags/polynomial.md#polynomial-mod-p-addsub-batch-simd-gnfs_poly_add_mod_simd) | auto | mod-p add/sub 批量 SIMD |
| [`GNFS_POLY_HORNER_MOD_SIMD`](docs/env-flags/polynomial.md#polynomial-modular-horner-batch-evaluation-simd-gnfs_poly_horner_mod_simd) | auto | mod-p Horner 多点求值 SIMD |
| [`GNFS_POLY_DIVREM_SUBQUADRATIC`](docs/env-flags/polynomial.md#polynomial-subquadratic-divrem-gnfs_poly_divrem_subquadratic) | auto | Newton-reciprocal subquadratic divrem |
| [`GNFS_POLY_HORNER_BATCH_SIMD`](docs/env-flags/polynomial.md#polynomial-horner-batch-evaluation-simd-gnfs_poly_horner_batch_simd) | auto | int64 Horner 多点求值 SIMD |

### 筛法 (sieve) — [完整文档](docs/env-flags/sieve.md)

| ENV | 默认 | 用途 |
|-----|------|------|
| [`GNFS_SIEVE_RESUME`](docs/env-flags/sieve.md#sieve-mid-flight-checkpoint-gnfs_sieve_resume) | off | sieve mid-flight checkpoint / resume |
| [`GNFS_BUCKET_PREFETCH`](docs/env-flags/sieve.md#sieve-bucket-prefetch-gnfs_bucket_prefetch) | auto | bucket scatter/gather prefetch |
| [`GNFS_SIEVE_REGION_TILE_BITS`](docs/env-flags/sieve.md#sieve-region-tile-bits-gnfs_sieve_region_tile_bits) | 0 | apply scan row tile cache blocking |
| [`GNFS_SIEVE_NORM_TILE_BITS`](docs/env-flags/sieve.md#sieve-norm-tile-bits-gnfs_sieve_norm_tile_bits) | 0 | norm 预计算 row tile cache blocking |
| [`GNFS_SIEVE_APPLY_TILE_THREADS`](docs/env-flags/sieve.md#sieve-apply-tile-并行-gnfs_sieve_apply_tile_threads) | 1 | apply-tile 并行 work distribution |
| [`GNFS_LATTICE_COORDS_SIMD`](docs/env-flags/sieve.md#lattice-coordinate-simd-batch-gnfs_lattice_coords_simd) | auto | lattice (i,j)→(a,b) 投影 SIMD |
| [`GNFS_SIEVE_COUNT_ABOVE_THRESHOLD_SIMD`](docs/env-flags/sieve.md#sieve-threshold-count-simd-gnfs_sieve_count_above_threshold_simd) | auto | sieve_array threshold count SIMD |
| [`GNFS_SIEVE_SATURATED_SUB_SIMD`](docs/env-flags/sieve.md#sieve-uint8-saturated-subtract-simd-gnfs_sieve_saturated_sub_simd) | auto | uint8 饱和减法 SIMD |
| [`GNFS_LATTICE_BASIS_PARALLEL_THREADS`](docs/env-flags/sieve.md#lattice-basis-reduction-多基并行-gnfs_lattice_basis_parallel_threads) | 1 | lattice basis reduction 多基并行 |

### 工具 (util) — [完整文档](docs/env-flags/util.md)

| ENV | 默认 | 用途 |
|-----|------|------|
| [`GNFS_INTEGER_SCRATCH_POOL`](docs/env-flags/util.md#integer-thread-local-scratch-pool-gnfs_integer_scratch_pool) | 0 | Integer thread-local scratch pool |
| [`GNFS_MPZ_POWM_BATCH_THREADS`](docs/env-flags/util.md#gmp-mpz_powm-批量并行-gnfs_mpz_powm_batch_threads) | 1 | batched mpz_powm 并行 |
| [`GNFS_MPZ_INVERT_BATCH_THREADS`](docs/env-flags/util.md#gmp-mpz_invert-批量并行-gnfs_mpz_invert_batch_threads) | 1 | batched mpz_invert 并行 |
| [`GNFS_MPZ_MOD_BATCH_THREADS`](docs/env-flags/util.md#gmp-mpz_mod-批量并行-gnfs_mpz_mod_batch_threads) | 1 | batched mpz_mod 并行 |
| [`GNFS_MPZ_GCD_BATCH_THREADS`](docs/env-flags/util.md#gmp-mpz_gcd-批量并行-gnfs_mpz_gcd_batch_threads) | 1 | batched mpz_gcd 并行 |
| [`GNFS_MPZ_MUL_BATCH_THREADS`](docs/env-flags/util.md#gmp-mpz_mul-批量并行-gnfs_mpz_mul_batch_threads) | 1 | batched mpz_mul 并行 |

### 平方根 (sqrt) — [完整文档](docs/env-flags/sqrt.md)

| ENV | 默认 | 用途 |
|-----|------|------|
| [`GNFS_SQRT_HENSEL_THREADS`](docs/env-flags/sqrt.md#hensel-lift-k-prime-slot-并行-gnfs_sqrt_hensel_threads) | 1 | Hensel lift K-prime slot 并行 |
| [`GNFS_COUVEIGNES_PARALLEL_THREADS`](docs/env-flags/sqrt.md#couveignes-pattern-search-并行-gnfs_couveignes_parallel_threads) | 1 | Couveignes sign-pattern search 并行 |

### 因子基 (factor_base) — [完整文档](docs/env-flags/factor_base.md)

| ENV | 默认 | 用途 |
|-----|------|------|
| [`GNFS_FB_ROOTS_THREADS`](docs/env-flags/factor_base.md#factor-base-cz-roots-并行-gnfs_fb_roots_threads) | 0=hw | Cantor-Zassenhaus 求根 per-prime 并行 |

## 设计细节文档化规范(强制)

AGENTS.md 只承载**全局操作规范 + 关键约定 / 铁律**。凡"特别具体的设计细节"——单个 `GNFS_*` 开关 / helper 的算法、ENV 解析规则、bit-for-bit 证明、ROI 分析、集成点、测试清单——一律写入 [`docs/`](docs/),AGENTS.md 仅留一句话速查 + 链接。

### 判据:写 AGENTS.md 还是写 docs?

- **写某段代码 / 跑某个实验时才查**的具体细节 → `docs/`
- **每次开发都要遵守**的流程 / 约定 / 铁律 → AGENTS.md

### 新增 `GNFS_*` 开关 / helper 时

1. **详细文档写到 [`docs/env-flags/<module>.md`](docs/env-flags/README.md)**(对应代码模块,追加一节),**不写进 AGENTS.md**。
2. **在上方「ENV 调优开关总览」对应模块子表加一行**:`` `GNFS_XXX` ``(带锚点链接)| 默认值 | 一句话用途。
3. **每节文档遵循统一模板**(与现有 62 节一致):标题 `## XXX (GNFS_XXX)` + 一句话定位 → 用法示例(bash)→ Helper API / 算法 → ENV 解析规则(严格,非法值 fallback)→ **Bit-for-bit guarantee** → ROI 与定位 → 集成点(文件 + commit)→ 测试清单 → **Default 行为**。
4. 若属 Parallel Dispatcher / SIMD family,同步更新 [`docs/env-flags/README.md`](docs/env-flags/README.md) 的 family 表。

### 其他设计细节归属

| 内容类型 | 去向 |
|----------|------|
| `GNFS_*` 开关 / helper 详细设计 | [`docs/env-flags/<module>.md`](docs/env-flags/README.md) |
| 算法设计 / 数学推导 | `docs/algorithms/` |
| 性能优化方案 / 实验记录 / 长设计文档 | `docs/perf/` |
| README / docs 写作规范 | [`docs/writing-style-guide.md`](docs/writing-style-guide.md) |
| 全局操作规范 + 关键铁律 | 本文件(AGENTS.md) |
...
撰写或修改 README / docs 设计文档 / 长 commit message body / PR 描述等正式文本前,**必读** [`docs/writing-style-guide.md`](docs/writing-style-guide.md):涵盖 GitHub Markdown 渲染规范、中英文混排、正规英语 / 中文写作规范、README 撰写检查清单、推荐工具链。
...
| `docs` | 文档 | `docs: update AGENTS.md with git conventions` |
...
- Codex 运行时：`.Codex/scheduled_tasks.lock`

### BACKLOG 自动修复策略（已授权）

**BACKLOG.md 中已记录的问题，Codex 被授权在二次确认错误确实存在后直接修复**，无需逐条等待用户手动审核。
...
**这是非公开私有仓库，Codex 被授权在每个 to-do 大阶段完成后自动 push**。
...
├── docs/plans/         # 设计文档
├── .Codex/            # Codex 配置 (agents, skills, hooks)
...
- Block Wiedemann 内存:Krylov 序列 O(n · L · 8 bytes),4 GB guard 限制。缓解见
  [`GNFS_BW_KRYLOV_MMAP` / `GNFS_BW_KRYLOV_STREAMS`](docs/env-flags/linalg.md)。
- MmapCSRMatrix Phase 5 集成尚未实施(需 SpMV API 泛化,multi-day surgery)。
- Murphy E rotation-incremental 算法重构 deferred(multi-day pure math);当前
  [`GNFS_MURPHY_ALPHA_THREADS`](docs/env-flags/polynomial.md) 并行是 orthogonal 加速。
- Sieve 内核 NEON 收益有限(gather / scatter 不适合 SIMD):Phase 0 global + v-prime
  row 已用 NEON 8-lane,bucket scatter 保持 scalar;E-core QoS 分离已 wire-in
  4 个 sieve thread spawn site。
- OOC 流式持久化、并行 dispatcher、SIMD 内核、cache / pool 等大量运行时调优开关已落地,
  完整文档见 [`docs/env-flags/`](docs/env-flags/README.md)。
