...
Linux 包在 Ubuntu 20.04 容器中使用 GCC 12 构建，并为 Release LTO 明确绑定同版本的 `gcc-ar`、`gcc-nm` 和 `gcc-ranlib`；Release Readiness 会在每个 PR 与 `main` SHA 上复现这条真实构建链。发布检查会拒绝高于 `GLIBC_2.31`、`GLIBCXX_3.4.30` 或 `CXXABI_1.3.13` 的符号版本，并拒绝未列入契约的动态库。归档内机器生成的 `binary-compatibility.json` 和 `README-release.txt` 会给出该二进制实际观察到的三个最低 ABI 版本；不能只依据 glibc 版本判断兼容性。macOS 包使用 `CMAKE_OSX_DEPLOYMENT_TARGET=13.0`，同时以 `lipo`、`vtool` 和 `otool` 确认单一 `arm64` 架构及 `minos 13.0`。
