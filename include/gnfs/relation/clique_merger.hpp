...
struct CliqueStats {
    ...
    [[nodiscard]] std::string to_string() const {
        return "in=" + std::to_string(input_relations) + " (1lp=" + std::to_string(input_1lp) +
               " 2lp=" + std::to_string(input_2lp) + " 3lp+=" + std::to_string(input_3lp_plus) +
               ")" + " components=" + std::to_string(components_with_excess) + "/" +
               std::to_string(components_found) + " full=" + std::to_string(full_produced) +
...
public:
    ...
    merge_cliques(std::vector<Relation> partials, CliqueStats* stats_out = nullptr,
        ...
        // ── 预过滤: 1LP + 2LP 总进 pool, 3LP+ 是否进取决于 frozen policy ──
        //
        ...
        //
        // 默认 policy OFF 时 3LP+ 仍 drop, 零回归. 启用 OPT-IN 时:
...
private:
    ...
    static void merge_component(
        std::vector<Relation>& pool, const std::vector<std::vector<LargePrimeKey>>& pool_lp_keys,
        const std::vector<size_t>& members,
        const std::unordered_map<LargePrimeKey, std::vector<size_t>, LargePrimeKeyHash>& lp_index,
        std::vector<bool>& used, std::vector<Relation>& results, CliqueStats& stats,
        bool drop_residual) {
        ...
        for (size_t start : members) {
            if (used[start] || visited.count(start))
                continue;
            ...
            while (!bfs.empty()) {
                ...
                bfs.pop();
                if (acc_full)
                    break; // accumulator 已满, 等下个 batch
                ...
                for (const auto& key : cur_keys) {
                    ...
                    for (size_t nbr : it->second) {
                        if (!in_component.count(nbr) || visited.count(nbr) || used[nbr])
                            continue;
                        ...
                        if (now_full) {
                            // is_effectively_full(acc) == cand_keys.empty() — saved a
                            // remaining_lp_keys call
                            acc_full = true;
                            break;
                        }
                    }
