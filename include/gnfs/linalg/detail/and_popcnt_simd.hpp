...
// SIMD-accelerated GF(2) batch AND-then-popcount helpers
// (ARM NEON + x86 AVX2).
//
// Scope
// -----
// This header provides batched popcount over the bitwise AND of two
// uint64_t word arrays, where the per-word output is the Hamming weight
// of `(a[i] & b[i])`. The dot-product over GF(2) of two packed bit
// vectors v, w is the parity of `total_and_popcount_words(v, w)`; the
// orthogonality check inside Block Lanczos / Block Wiedemann amounts
// to verifying that this total has the expected parity for every pair
// of basis vectors. Both helpers are the AND-fused counterpart of the
// `popcount_simd.hpp` plain popcount batch.
//
// The scalar fallback uses `__builtin_popcountll(a[i] & b[i])`
// (single-AND + single-CNT on M-series ARM64 and on x86_64 with
// `-mpopcnt`); the SIMD path batches 16 bytes at a time so the
// compiler can schedule independent NEON `vandq_u64 + vcntq_u8` lanes
// (or AVX2 256-bit `_mm256_and_si256` followed by
// `_mm256_popcnt_epi64` when the AVX-512 popcount extension is
// available, otherwise a 4-wide `_mm_popcnt_u64` unroll).
//
...
// into wider pipelines (e.g. Block Lanczos / Block Wiedemann
// orthogonality check, parity dot-product reduction, GF(2) inner
// product) without paying for thread pool setup.
//
...
// reduction in `total_and_popcount_words` is an integer sum of pure
...
// `and_popcnt_simd_supported()` returns false and the dispatcher
...
#include "../../util/bit_intrin.hpp"
    ...
    #include <arm_neon.h>
    #define GNFS_AND_POPCNT_SIMD_NEON 1
  #endif
#endif
    ...
    #include <immintrin.h>
    #define GNFS_AND_POPCNT_SIMD_AVX2 1
  #endif
#endif

namespace gnfs::linalg::detail {

/// Three-state ENV gate matching the `GNFS_GF2_POPCNT_SIMD` /
/// `GNFS_SPMV_SIMD` / `GNFS_BUCKET_PREFETCH` convention. `Auto` defers
/// to compile-time SIMD availability, `ForceOff` forces the scalar
/// path (useful for regression-bisect or sanitizer runs that want to
/// isolate kernel changes from SIMD-specific noise), and `ForceOn`
/// opts in even when `and_popcnt_simd_supported()` is false — in
/// which case the dispatcher still falls back to scalar to keep
/// correctness.
enum class AndPopcntSimdMode : std::uint8_t {
    Auto = 0,
    ForceOff = 1,
    ForceOn = 2,
};

namespace and_popcnt_detail {

inline std::atomic<std::uint8_t>& cached_state() noexcept {
    static std::atomic<std::uint8_t> state{static_cast<std::uint8_t>(AndPopcntSimdMode::Auto)};
    return state;
}
...
inline AndPopcntSimdMode resolve_mode_from_env() noexcept {
    const char* v = std::getenv("GNFS_GF2_AND_POPCNT_SIMD");
    if (v == nullptr) return AndPopcntSimdMode::Auto;
    if (std::strcmp(v, "0") == 0) return AndPopcntSimdMode::ForceOff;
    if (std::strcmp(v, "1") == 0) return AndPopcntSimdMode::ForceOn;
    // "auto" or anything else
    return AndPopcntSimdMode::Auto;
}

inline AndPopcntSimdMode load_mode() noexcept {
    std::call_once(cached_flag(), []() noexcept {
        cached_state().store(static_cast<std::uint8_t>(resolve_mode_from_env()),
                             std::memory_order_relaxed);
    });
    return static_cast<AndPopcntSimdMode>(cached_state().load(std::memory_order_relaxed));
}
...
[[nodiscard]] constexpr bool and_popcnt_simd_supported() noexcept {
#if defined(GNFS_AND_POPCNT_SIMD_NEON) || defined(GNFS_AND_POPCNT_SIMD_AVX2)
    return true;
#else
    return false;
#endif
...
[[nodiscard]] inline AndPopcntSimdMode and_popcnt_simd_mode() noexcept {
    return and_popcnt_detail::load_mode();
}
...
[[nodiscard]] inline bool and_popcnt_simd_enabled() noexcept {
    const AndPopcntSimdMode mode = and_popcnt_simd_mode();
    if (mode == AndPopcntSimdMode::ForceOff) return false;
    // ForceOn and Auto both require compile-time availability.
    return and_popcnt_simd_supported();
}

/// Re-read GNFS_GF2_AND_POPCNT_SIMD from the environment and refresh
/// the cached gate. Intended for unit tests that flip the env between
/// scenarios. Not called from production hot paths.
inline void and_popcnt_simd_reset_env_cache_for_testing() noexcept {
    and_popcnt_detail::cached_state().store(
        static_cast<std::uint8_t>(and_popcnt_detail::resolve_mode_from_env()),
        std::memory_order_relaxed);
    // Ensure call_once records completion even if it has never run,
    // so a later production call does not overwrite our state.
    std::call_once(and_popcnt_detail::cached_flag(), []() noexcept {});
}
...
/// using the compiler builtin. O(n) ops; one AND + one CNT per word
...
/// output buffer.
inline void batch_and_popcount_words_scalar(std::span<const std::uint64_t> a,
...
total_and_popcount_words_scalar(std::span<const std::uint64_t> a,
...
#if defined(GNFS_AND_POPCNT_SIMD_NEON)
/// NEON inner kernel: AND two pairs of 64-bit words, popcount each
/// resulting byte via `vcntq_u8`, then horizontal-sum each word's
/// 8-byte half with `vaddv_u8`. Two output words per call.
inline void and_popcount_pair_neon(const std::uint64_t* a_in,
...
/// NEON path for the total sum: accumulate 2-word AND-popcounts in a
/// uint64_t total. Single horizontal sum across 16 bytes per
/// Q-register load.
[[nodiscard]] inline std::uint64_t total_and_popcount_neon(std::span<const std::uint64_t> a,
                                                           std::span<const std::uint64_t> b) noexcept {
    const std::size_t n = (a.size() < b.size()) ? a.size() : b.size();
...
#endif  // GNFS_AND_POPCNT_SIMD_NEON

#if defined(GNFS_AND_POPCNT_SIMD_AVX2)
/// AVX2 inner kernel: AND four 64-bit words, popcount each via either
/// `_mm256_popcnt_epi64` (AVX-512 VPOPCNTDQ + VL) or a 4-wide
/// `_mm_popcnt_u64` fallback. Four output words per call.
inline void and_popcount_quad_avx2(const std::uint64_t* a_in,
                                   const std::uint64_t* b_in,
                                   std::uint32_t* out) noexcept {
    ...
    __m256i pc = _mm256_popcnt_epi64(vand);
    ...
    _mm256_storeu_si256(reinterpret_cast<__m256i*>(lanes), vand);
...
[[nodiscard]] inline std::uint64_t total_and_popcount_avx2(std::span<const std::uint64_t> a,
                                                           std::span<const std::uint64_t> b) noexcept {
    const std::size_t n = (a.size() < b.size()) ? a.size() : b.size();
    const std::uint64_t* pa = a.data();
    const std::uint64_t* pb = b.data();
...
#endif  // GNFS_AND_POPCNT_SIMD_AVX2
...
/// Batch AND-popcount: `out[i] = popcount(a[i] & b[i])` for `i in
/// [0, a.size())`. Pre-conditions: `a.size() == b.size()` and
/// `out.size() >= a.size()`. Empty input returns without touching
/// any span. SIMD path is taken when `and_popcnt_simd_enabled()` is
...
inline void batch_and_popcount_words(std::span<const std::uint64_t> a,
                                     std::span<const std::uint64_t> b,
                                     std::span<std::uint32_t> out) noexcept {
    assert(a.size() == b.size() && "a.size() must equal b.size()");
    const std::size_t n_ab = (a.size() < b.size()) ? a.size() : b.size();
...
/// Total AND-popcount: returns the sum of `popcount(a[i] & b[i])`
/// over all `i`. Empty input returns zero. SIMD path is taken when
/// `and_popcnt_simd_enabled()` is true; otherwise falls back to the
/// scalar reference. Bit-for-bit identical total across both paths.
///
/// Pre-condition: `a.size() == b.size()` (asserted in debug builds).
/// If sizes diverge the implementation clamps to the shorter span.
[[nodiscard]] inline std::uint64_t
total_and_popcount_words(std::span<const std::uint64_t> a,
                         std::span<const std::uint64_t> b) noexcept {
    assert(a.size() == b.size() && "a.size() must equal b.size()");
    if (a.empty() || b.empty()) return 0;
    if (!and_popcnt_simd_enabled()) {
        return total_and_popcount_words_scalar(a, b);
    }
#if defined(GNFS_AND_POPCNT_SIMD_NEON)
    return and_popcnt_detail::total_and_popcount_neon(a, b);
#elif defined(GNFS_AND_POPCNT_SIMD_AVX2)
    return and_popcnt_detail::total_and_popcount_avx2(a, b);
#else
    return total_and_popcount_words_scalar(a, b);
#endif
}
