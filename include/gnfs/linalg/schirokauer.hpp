...
private:
    ...
    void precompute_for_prime(uint32_t ell) {
        ...
        // The hot paths accept signed a values and currently reduce them by
        // converting ell_k to int64_t. Reject values that cannot be represented
        // there instead of relying on an implementation-defined narrowing cast.
        ...
        // Compute f(x) mod ℓ^k. Use GMP's arbitrary-precision modulus rather
        // than mpz_fdiv_ui, whose unsigned long modulus is only 32 bits on
        // Windows LLP64 builds.
