...
struct BaiBrentParams {
    ...
    [[nodiscard]] static BaiBrentParams from_gnfs_params(const GNFSParams& gp) {
        ...
        bp.search_radius = static_cast<uint32_t>(
        ...
        bp.murphy_params.alpha_bound = std::min(
        ...
        bp.murphy_params.sample_points =
            static_cast<uint32_t>(std::clamp(sp_cont, 100.0, 5000.0));
...
private:
    ...
    stage1_leading_coeff_search(const Integer& n) {
        ...
        for (const auto& ad : ad_list) {
            ...
            for (int32_t delta = -static_cast<int32_t>(params_.search_radius);
                ...
                if (!g.fits_uint64() || g.to_uint64() != 1) continue;
    ...
    [[nodiscard]] std::vector<Integer> generate_ad_candidates(
        ...
        if (params_.smooth_preference) {
            ...
            for (uint32_t p : small_primes) {
                ...
                for (size_t i = 0; i < cur_size; ++i) {
                    ...
                    while (v <= hi / p) {
                        v *= p;
                        if (v >= lo && v <= hi) smooth.push_back(v);
                    }
                }
            }
    ...
    [[nodiscard]] bool is_valid_polynomial(
        ...
        if (f.degree() != params_.degree) return false;
        if (f.leading_coeff().is_zero()) return false;
...
[[nodiscard]] inline PolynomialContext create_context_from_bai_brent(
        const Integer& n, const BaiBrentResult& result) {
