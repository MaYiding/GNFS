...
materialize_two_large_prime_cycle(
    ...
    std::vector<size_t> relation_indices(
        cycle_relation_indices.begin(), cycle_relation_indices.end());
    ...
    for (const size_t relation_index : relation_indices) {
        ...
        if (position == source_order.end() ||
            sources[*position].relation_index != relation_index) {
            return std::nullopt;
        }
        selected_sources.push_back(&sources[*position]);
    }

    const size_t exponent_count =
        selected_sources.front()->factor_base_exponents.size();
    ...
    for (const auto* source : selected_sources) {
        ...
        for (size_t i = 0; i < exponent_count; ++i) {
            ...
            if (addend > std::numeric_limits<uint32_t>::max() -
                              factor_base_exponents[i]) {
                return std::nullopt;
            }
            factor_base_exponents[i] += addend;
        }

        mpz_mul(value_modulus.get_mpz(), value_modulus.get_mpz(),
                source->value.get_mpz());
        ...
        mpz_mod(value_modulus.get_mpz(), value_modulus.get_mpz(),
                modulus.get_mpz());
    ...
    for (size_t begin = 0; begin < large_prime_incidence.size();) {
        ...
        large_prime_square_roots.insert(
            large_prime_square_roots.end(), degree / 2,
