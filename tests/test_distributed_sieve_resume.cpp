...
static_assert(
    !std::is_copy_constructible_v<worker_launcher_detail::DistributedSieveWorkerLaunchSlotV1>);
static_assert(
    !std::is_copy_assignable_v<worker_launcher_detail::DistributedSieveWorkerLaunchSlotV1>);
static_assert(std::is_nothrow_move_constructible_v<
              worker_launcher_detail::DistributedSieveWorkerLaunchSlotV1>);
static_assert(
    !std::is_move_assignable_v<worker_launcher_detail::DistributedSieveWorkerLaunchSlotV1>);
static_assert(!std::is_default_constructible_v<
              worker_launcher_detail::DistributedSieveWorkerLaunchRequestV1>);
static_assert(
    !std::is_copy_constructible_v<worker_launcher_detail::DistributedSieveWorkerLaunchRequestV1>);
static_assert(std::is_nothrow_move_constructible_v<
              worker_launcher_detail::DistributedSieveWorkerLaunchRequestV1>);
static_assert(!std::is_default_constructible_v<
              worker_launcher_detail::DistributedSieveLaunchedWorkerAttemptV1>);
static_assert(
    !std::is_copy_constructible_v<worker_launcher_detail::DistributedSieveLaunchedWorkerAttemptV1>);
static_assert(std::is_nothrow_move_constructible_v<
              worker_launcher_detail::DistributedSieveLaunchedWorkerAttemptV1>);
static_assert(
    !std::is_constructible_v<worker_launcher_detail::DistributedSieveLaunchedWorkerAttemptV1,
                             WorkerAttemptStartReceipt&&>);
static_assert(
...
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_BOOTSTRAP_VALID = 1U << 0U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_MANIFEST_VALID = 1U << 1U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_ROOT_IDENTITY_VALID = 1U << 2U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_WAVE_LOCK_IDENTITY_VALID = 1U << 3U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_BASE_LOCK_IDENTITY_VALID = 1U << 4U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_WAVE_LOCK_SAME_OFD = 1U << 5U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_BASE_LOCK_SAME_OFD = 1U << 6U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_PACKAGE_METADATA_VALID = 1U << 7U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_PACKAGE_IDENTITY_VALID = 1U << 8U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_FIXED_FDS_NOT_CLOEXEC = 1U << 9U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_NO_DESCRIPTOR_LEAK = 1U << 10U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_BASE_LOCK_CLOSED = 1U << 11U;
inline constexpr std::uint32_t WORKER_LAUNCH_CHILD_REQUIRED_FLAGS =
    WORKER_LAUNCH_CHILD_BOOTSTRAP_VALID | WORKER_LAUNCH_CHILD_MANIFEST_VALID |
    WORKER_LAUNCH_CHILD_ROOT_IDENTITY_VALID | WORKER_LAUNCH_CHILD_WAVE_LOCK_IDENTITY_VALID |
    WORKER_LAUNCH_CHILD_BASE_LOCK_IDENTITY_VALID | WORKER_LAUNCH_CHILD_WAVE_LOCK_SAME_OFD |
    WORKER_LAUNCH_CHILD_BASE_LOCK_SAME_OFD | WORKER_LAUNCH_CHILD_PACKAGE_METADATA_VALID |
    WORKER_LAUNCH_CHILD_PACKAGE_IDENTITY_VALID | WORKER_LAUNCH_CHILD_FIXED_FDS_NOT_CLOEXEC |
    WORKER_LAUNCH_CHILD_NO_DESCRIPTOR_LEAK;

struct WorkerLaunchFakeChildReport final {
    std::uint32_t magic = WORKER_LAUNCH_FAKE_CHILD_REPORT_MAGIC;
    std::uint32_t flags = 0;
    std::uint32_t chunk_id = 0;
    std::uint32_t attempt_ordinal = 0;
    std::int32_t first_unexpected_descriptor = -1;
    std::int32_t native_error = 0;
    Digest manifest_digest;
    Digest work_digest;
};

static_assert(std::is_trivially_copyable_v<WorkerLaunchFakeChildReport>);

[[nodiscard]] bool same_native_identity(const sieve::NativeIdentityV1& expected,
                                        const struct stat& actual) noexcept {
    return expected.volume == static_cast<std::uint64_t>(actual.st_dev) &&
           expected.object == static_cast<std::uint64_t>(actual.st_ino) && expected.generation == 0;
}

[[nodiscard]] bool read_stream_bounded(int descriptor, std::vector<std::byte>& bytes,
    ...
    for (;;) {
        ...
        if (count > limit - std::min(limit, bytes.size())) {
            return false;
        }
        try {
...
[[nodiscard]] bool read_regular_file_exact(int descriptor, std::vector<std::byte>& bytes) noexcept {
    ...
    while (consumed < bytes.size()) {
        ssize_t received = -1;
        do {
            received = ::pread(descriptor, bytes.data() + consumed, bytes.size() - consumed,
...
[[nodiscard]] bool write_exact_noexcept(int descriptor, const void* source,
...
[[nodiscard]] bool descriptor_retains_exclusive_lock(int descriptor) noexcept {
    int result = -1;
    do {
...
[[nodiscard]] bool fixed_descriptors_are_not_cloexec() noexcept {
...
[[nodiscard]] int first_open_worker_descriptor() noexcept {
    for (int descriptor = 7; descriptor < WORKER_LAUNCH_FAKE_CHILD_DESCRIPTOR_SCAN_LIMIT;
         ++descriptor) {
        errno = 0;
...
[[nodiscard]] int run_worker_launch_fake_child(bool close_base_lock_and_wait = false) noexcept {
    WorkerLaunchFakeChildReport report;
    std::vector<std::byte> attempt_bytes;
    if (!read_stream_bounded(STDIN_FILENO, attempt_bytes, WORKER_LAUNCH_FAKE_CHILD_INPUT_LIMIT)) {
        report.native_error = errno;
        (void)write_exact_noexcept(STDOUT_FILENO, &report, sizeof(report));
        return 210;
    }

    const auto attempt_decoded = sieve::decode_distributed_sieve_record(attempt_bytes);
    const auto* attempt = attempt_decoded && attempt_decoded.value.has_value()
                              ? std::get_if<sieve::AttemptStartedV1>(&*attempt_decoded.value)
                              : nullptr;
    if (attempt == nullptr) {
        (void)write_exact_noexcept(STDOUT_FILENO, &report, sizeof(report));
        return 211;
    }
    report.flags |= WORKER_LAUNCH_CHILD_BOOTSTRAP_VALID;
    report.chunk_id = attempt->chunk_id;
    report.attempt_ordinal = attempt->attempt_ordinal;
    report.manifest_digest = attempt->manifest_digest;

    const std::string manifest_leaf(wave_detail::DISTRIBUTED_SIEVE_WAVE_MANIFEST_LEAF);
    int manifest_fd = -1;
    do {
        manifest_fd = ::openat(3, manifest_leaf.c_str(), O_RDONLY | O_NOFOLLOW | O_CLOEXEC);
    } while (manifest_fd < 0 && errno == EINTR);
    if (manifest_fd < 0) {
        report.native_error = errno;
        (void)write_exact_noexcept(STDOUT_FILENO, &report, sizeof(report));
        return 212;
    ...
    const bool manifest_read = read_regular_file_exact(manifest_fd, manifest_bytes);
    const int manifest_close_result = ::close(manifest_fd);
    if (!manifest_read || manifest_close_result != 0) {
        report.native_error = errno;
        (void)write_exact_noexcept(STDOUT_FILENO, &report, sizeof(report));
        return 213;
    }
    const auto manifest_decoded = sieve::decode_distributed_sieve_record(manifest_bytes);
    const auto* manifest = manifest_decoded && manifest_decoded.value.has_value()
                               ? std::get_if<sieve::WaveManifestV1>(&*manifest_decoded.value)
                               : nullptr;
    if (manifest == nullptr || manifest->self_digest != attempt->manifest_digest) {
        (void)write_exact_noexcept(STDOUT_FILENO, &report, sizeof(report));
        return 214;
    }

    const auto chunk =
        std::ranges::find(manifest->chunks, attempt->chunk_id, &sieve::ChunkPlanV1::chunk_id);
    if (chunk == manifest->chunks.end() || chunk->sq_begin != attempt->sq_begin ||
        chunk->sq_end != attempt->sq_end) {
        (void)write_exact_noexcept(STDOUT_FILENO, &report, sizeof(report));
        return 215;
    }
    ...
    if (S_ISDIR(root_metadata.st_mode) &&
        same_native_identity(manifest->wave_root_identity, root_metadata)) {
        report.flags |= WORKER_LAUNCH_CHILD_ROOT_IDENTITY_VALID;
    }
    if (S_ISREG(wave_lock_metadata.st_mode) &&
        same_native_identity(manifest->permanent_lock_identity, wave_lock_metadata)) {
        report.flags |= WORKER_LAUNCH_CHILD_WAVE_LOCK_IDENTITY_VALID;
    }

    const auto names = wave_detail::distributed_sieve_worker_attempt_names_v1(
        chunk->relative_artifact_stem, chunk->chunk_id, attempt->attempt_ordinal);
    if (!names.has_value() || names->relative_lease_stem != attempt->lease.relative_stem) {
        (void)write_exact_noexcept(STDOUT_FILENO, &report, sizeof(report));
        return 217;
    }
    const std::string base_lock_leaf(names->base_lock_leaf);
    if (::fstatat(3, base_lock_leaf.c_str(), &named_base_lock_metadata, AT_SYMLINK_NOFOLLOW) == 0 &&
        S_ISREG(base_lock_metadata.st_mode) &&
        base_lock_metadata.st_dev == named_base_lock_metadata.st_dev &&
        base_lock_metadata.st_ino == named_base_lock_metadata.st_ino) {
        report.flags |= WORKER_LAUNCH_CHILD_BASE_LOCK_IDENTITY_VALID;
    }
    if (descriptor_retains_exclusive_lock(4)) {
        report.flags |= WORKER_LAUNCH_CHILD_WAVE_LOCK_SAME_OFD;
    }
    if (descriptor_retains_exclusive_lock(5)) {
        report.flags |= WORKER_LAUNCH_CHILD_BASE_LOCK_SAME_OFD;
    }

    int package_status_flags = -1;
    do {
        package_status_flags = ::fcntl(6, F_GETFL);
    } while (package_status_flags < 0 && errno == EINTR);
    if (package_status_flags >= 0 && S_ISREG(package_metadata.st_mode) &&
        package_metadata.st_nlink == 0 &&
        (package_metadata.st_mode & static_cast<mode_t>(07777)) == 0400 &&
        (package_status_flags & O_ACCMODE) == O_RDONLY) {
        report.flags |= WORKER_LAUNCH_CHILD_PACKAGE_METADATA_VALID;
    }

    std::vector<std::byte> package_bytes;
    if (!read_regular_file_exact(6, package_bytes)) {
        report.native_error = errno;
        (void)write_exact_noexcept(STDOUT_FILENO, &report, sizeof(report));
        return 218;
    }
    const auto package = gnfs::sieve::distributed_sieve_work_package_codec_detail::
        decode_distributed_sieve_work_package_v1(package_bytes);
    if (package && package.package.has_value()) {
        const auto work_digest = sieve::distributed_sieve_work_digest(package.package->identity);
        if (work_digest && work_digest.digest.has_value() &&
            *work_digest.digest == manifest->work_sha256 &&
            package.package->witness.work_sha256 == manifest->work_sha256) {
            report.flags |= WORKER_LAUNCH_CHILD_PACKAGE_IDENTITY_VALID;
            report.work_digest = *work_digest.digest;
        }
    }
    if (fixed_descriptors_are_not_cloexec()) {
        report.flags |= WORKER_LAUNCH_CHILD_FIXED_FDS_NOT_CLOEXEC;
    }
    report.first_unexpected_descriptor = first_open_worker_descriptor();
    if (report.first_unexpected_descriptor < 0) {
        report.flags |= WORKER_LAUNCH_CHILD_NO_DESCRIPTOR_LEAK;
    }

    const bool required_valid =
        (report.flags & WORKER_LAUNCH_CHILD_REQUIRED_FLAGS) == WORKER_LAUNCH_CHILD_REQUIRED_FLAGS;
    if (close_base_lock_and_wait) {
        const int close_result = ::close(5);
        errno = 0;
        if (close_result == 0 && ::fcntl(5, F_GETFD) < 0 && errno == EBADF) {
            report.flags |= WORKER_LAUNCH_CHILD_BASE_LOCK_CLOSED;
        }
        if (!write_exact_noexcept(STDOUT_FILENO, &report, sizeof(report))) {
            return 219;
        }
        if (!required_valid || (report.flags & WORKER_LAUNCH_CHILD_BASE_LOCK_CLOSED) == 0U) {
            return 220;
        }
        for (;;) {
            (void)::pause();
...
worker_launch_frozen_policy() {
    ...
    if (!frozen || !frozen.policy.has_value()) {
        fail("freeze worker-launch execution policy", __LINE__,
             sieve::distributed_sieve_protocol_error_name(frozen.status.error));
    }
    return std::move(*frozen.policy);
}

[[nodiscard]] sieve::DistributedSieveWorkIdentityV1 worker_launch_work_identity(
    const execution_policy_detail::DistributedSieveFrozenExecutionPolicyV1& frozen,
    std::uint32_t worker_count = 3) {
    if (worker_count == 0 || worker_count > 3) {
        fail("worker-launch fixture count", __LINE__);
    }
    auto identity = make_work_identity();
    ...
    identity.distributed.worker_count = worker_count;
    identity.distributed.chunks.clear();
    identity.distributed.chunks.reserve(worker_count);
    for (std::uint32_t index = 0; index < worker_count; ++index) {
        const auto begin = static_cast<std::uint32_t>(2U + index);
        identity.distributed.chunks.push_back(
            sieve::ChunkPlanV1{index, begin, static_cast<std::uint32_t>(begin + 1U),
                               "launch_chunk_" + std::to_string(index)});
    }
    identity.original_sq_bounds.start_index = 0;
    identity.original_sq_bounds.end_index = static_cast<std::uint32_t>(2U + worker_count);
    identity.effective_sq_bounds.start_index = 2;
    identity.effective_sq_bounds.end_index = static_cast<std::uint32_t>(2U + worker_count);
    identity.effective_sq_bounds.min_q = 0;
    identity.effective_sq_bounds.max_q = std::numeric_limits<std::uint32_t>::max();
    require_ok(sieve::validate_distributed_sieve_work_identity(identity),
               "worker-launch work identity");
    return identity;
}

[[nodiscard]] gnfs::core::PolynomialContext worker_launch_live_polynomial() {
    std::vector<gnfs::core::Integer> coefficients;
    coefficients.emplace_back("-5");
    coefficients.emplace_back("3");
    coefficients.emplace_back("1");
    return gnfs::core::PolynomialContext{gnfs::core::Integer("1000036000099"),
                                         std::move(coefficients), gnfs::core::Integer("10001"),
                                         1.25};
}

[[nodiscard]] gnfs::factor_base::FactorBase worker_launch_live_factor_base() {
    gnfs::factor_base::FactorBase factor_base(gnfs::core::FactorBaseParams{100, 200, 10'000, 16});
    factor_base.reserve(2, 5);
    ...
    factor_base.add_algebraic(211, 3, 61, 1);
    factor_base.add_algebraic(223, std::numeric_limits<std::uint32_t>::max(), 67, 1);
    factor_base.add_algebraic(227, 5, 71, 1);
...
worker_launch_manifest_draft(const sieve::DistributedSieveWorkIdentityV1& identity) {
    auto draft = wave_manifest_draft();
    draft.work_sha256 = work_digest_or_fail(identity);
    draft.effective_sq_begin = identity.distributed.chunks.front().sq_begin;
    draft.effective_sq_end = identity.distributed.chunks.back().sq_end;
    draft.worker_count = identity.distributed.worker_count;
    draft.chunks = identity.distributed.chunks;
    draft.sq_cap_per_worker = identity.distributed.sq_cap_per_worker;
    draft.relation_cap_per_worker = identity.distributed.relation_cap_per_worker;
    draft.max_worker_attempts = identity.distributed.max_worker_attempts;
    draft.max_merge_build_attempts = identity.distributed.max_merge_build_attempts;
    draft.max_consumption_attempts = identity.distributed.max_consumption_attempts;
    draft.canonical_naming_version = identity.semantic_versions.chunking_version;
    draft.retry_policy_version = identity.semantic_versions.retry_policy_version;
    draft.ooc_format_version = identity.semantic_versions.ooc_format_version;
    draft.relation_serialization_version =
        identity.semantic_versions.relation_serialization_version;
    draft.handoff_version = identity.semantic_versions.handoff_version;
    draft.receipt_version = identity.semantic_versions.completion_version;
    draft.digest_version = identity.semantic_versions.digest_version;
    draft.merge_policy_version = identity.semantic_versions.merge_policy_version;
    return draft;
}
...
struct WavePrivateLeaseRecoveryStopContext final {
    wave_detail::DistributedSievePrivateLeaseReservationBoundary target =
        wave_detail::DistributedSievePrivateLeaseReservationBoundary::PermitAcquired;
    std::array<bool, wave_detail::DISTRIBUTED_SIEVE_PRIVATE_LEASE_RESERVATION_BOUNDARIES.size()>
        observed{};
...
[[nodiscard]] bool stop_at_wave_private_lease_recovery_boundary(
    wave_detail::DistributedSievePrivateLeaseReservationBoundary boundary, void* opaque) noexcept {
    auto& context = *static_cast<WavePrivateLeaseRecoveryStopContext*>(opaque);
    const auto index = static_cast<std::size_t>(boundary);
    if (index < context.observed.size()) {
        context.observed[index] = true;
    }
...
void require_wave_attempt_inventory_reopens(wave_detail::DistributedSieveWaveStoreOpenResult& owner,
                                            const std::filesystem::path& root,
                                            std::string_view context) {
    auto& store = require_wave_ready(owner, context);
    const Digest manifest_digest = store.manifest_digest();
    const auto before = capture_wave_root_snapshot(root);
    require_wave_status(store.revalidate(), wave_detail::DistributedSieveWaveStoreStatus::ready,
                        context);
    ...
    owner.store.reset();
    auto reopened = wave_detail::DistributedSieveWaveStore::open(root, manifest_digest);
    auto& reopened_store = require_wave_ready(reopened, context);
...
void test_wave_store_private_lease_recovery_from_every_fresh_prefix() {
    using Boundary = wave_detail::DistributedSievePrivateLeaseReservationBoundary;
    ...
    for (std::size_t index = 1;
         ...
         ++index) {
        WaveStoreTempDirectory temp;
        const auto root = temp.path() / ("private-lease-recovery-prefix-" + std::to_string(index));
        auto created = wave_detail::DistributedSieveWaveStore::create(root, wave_manifest_draft());
        auto& store = require_wave_ready(created, "create private-lease recovery fixture");
        const auto& chunk = store.manifest().chunks.front();
        const auto names = wave_detail::distributed_sieve_worker_attempt_names_v1(
            chunk.relative_artifact_stem, chunk.chunk_id, 0);
        CHECK(names.has_value());
        ...
        auto claimed = store.create_worker_attempt_private_lease_root(chunk.chunk_id, 0);
        (void)require_private_lease_root_claim_ready(claimed, "create recovery fixture BaseLock");
        const auto p0_snapshot = capture_wave_root_snapshot(root);

        WavePrivateLeaseProtocolStopContext reservation_stop{
            .target = wave_detail::DISTRIBUTED_SIEVE_PRIVATE_LEASE_RESERVATION_BOUNDARIES[index],
        };
        auto interrupted_reservation = wave_detail::reserve_worker_attempt_private_lease(
            std::move(claimed), wave_detail::DistributedSievePrivateLeaseProtocolTestHooks{
                                    .stop_after = stop_at_wave_private_lease_boundary,
                                    .context = &reservation_stop,
                                });
        CHECK(!interrupted_reservation);
        require_wave_status(interrupted_reservation.diagnostic,
                            wave_detail::DistributedSieveWaveStoreStatus::interrupted,
                            "fresh writer leaves the requested recovery prefix");

        WaveReservationWitnessObservationContext initial_observation{
            .expected_boundary =
                wave_detail::DISTRIBUTED_SIEVE_PRIVATE_LEASE_RESERVATION_BOUNDARIES[index],
            .expected_base_lock_leaf = names->base_lock_leaf,
        };
        require_wave_status(
            ...
            wave_detail::DistributedSieveWaveStoreStatus::ready,
            "recovery fixture starts at an exact fresh-writer prefix");
        CHECK(initial_observation.invoked);
        CHECK(initial_observation.matched);
        ...
        auto create_only = store.create_worker_attempt_private_lease_root(chunk.chunk_id, 0);
        CHECK(!create_only);
        CHECK(create_only.claim == nullptr);
        require_wave_status(create_only.diagnostic,
                            wave_detail::DistributedSieveWaveStoreStatus::namespace_conflict,
                            "recovery prefix rejects the create-only entry point");
        CHECK(capture_wave_root_snapshot(root) == prefix_snapshot);
        if (nested_owner_snapshot.has_value()) {
            CHECK(capture_wave_root_entry_snapshot(nested_owner_snapshot->first,
                                                   nested_owner_snapshot->second.leaf) ==
        ...
        (void)require_private_lease_root_claim_ready(opened,
                                                     "open exact prefix for one recovery edge");
        WavePrivateLeaseRecoveryStopContext recovery_stop{
            .target =
                wave_detail::DISTRIBUTED_SIEVE_PRIVATE_LEASE_RESERVATION_BOUNDARIES[index - 1U],
        };
        auto interrupted_recovery = wave_detail::recover_worker_attempt_private_lease(
            std::move(opened), wave_detail::DistributedSievePrivateLeaseRecoveryTestHooks{
                                   .stop_after = stop_at_wave_private_lease_recovery_boundary,
                                   .context = &recovery_stop,
                               });
        CHECK(!interrupted_recovery);
        CHECK(interrupted_recovery.claim == nullptr);
        CHECK(opened.claim == nullptr);
        const std::string interrupted_context =
            "recovery stops only after exact reverse successor P" + std::to_string(index - 1U);
        require_wave_status(interrupted_recovery.diagnostic,
                            wave_detail::DistributedSieveWaveStoreStatus::interrupted,
                            interrupted_context);
        CHECK(interrupted_recovery.diagnostic.last_private_lease_recovery_boundary ==
              recovery_stop.target);
        for (std::size_t observed = 0; observed < recovery_stop.observed.size(); ++observed) {
            CHECK(recovery_stop.observed[observed] == (observed == index - 1U));
        }

        WaveReservationWitnessObservationContext successor_observation{
            .expected_boundary = recovery_stop.target,
            .expected_base_lock_leaf = names->base_lock_leaf,
        };
        require_wave_status(
            store.revalidate(wave_detail::DistributedSieveWaveStoreInventoryTestHooks{
                .observe_reservation_witnesses = observe_wave_reservation_witnesses,
                .context = &successor_observation,
            }),
            wave_detail::DistributedSieveWaveStoreStatus::ready,
            "interrupted recovery leaves its exact durable successor");
        CHECK(successor_observation.invoked);
        CHECK(successor_observation.matched);
        ...
        auto& recovered_claim = require_private_lease_root_claim_ready(
            recovered, "reopen and converge the recovery prefix to P0");
        require_wave_status(recovered_claim.revalidate(),
                            wave_detail::DistributedSieveWaveStoreStatus::ready,
                            "recovered claim carries the refreshed P0 witness");
        ...
        require_wave_status(
            ...
            wave_detail::DistributedSieveWaveStoreStatus::ready,
            "completed recovery is an exact P0 prefix");
        CHECK(p0_observation.invoked);
        CHECK(p0_observation.matched);
        CHECK(relation_base_lock_reports_busy(root / names->base_lock_leaf));

        auto root_busy = store.claim_private_lease_root();
        CHECK(!root_busy);
        require_wave_status(root_busy.diagnostic,
                            wave_detail::DistributedSieveWaveStoreStatus::private_lease_root_busy,
                            "returned recovery claim retains the same-State root slot");

        auto reserved = wave_detail::reserve_worker_attempt_private_lease(std::move(recovered));
        CHECK(reserved);
        CHECK(reserved.receipt.has_value());
        CHECK(recovered.claim == nullptr);
        require_wave_status(reserved.receipt->revalidate(),
                            wave_detail::DistributedSieveWaveStoreStatus::ready,
                            "recovered P0 claim enters the fresh forward protocol");
    }
}

void test_wave_store_private_lease_recovery_requires_open_existing_origin() {
    WaveStoreTempDirectory temp;
    const auto root = temp.path() / "private-lease-recovery-origin";
    auto created = wave_detail::DistributedSieveWaveStore::create(root, wave_manifest_draft());
    auto& store = require_wave_ready(created, "create recovery-origin fixture");
    const auto& chunk = store.manifest().chunks.front();
    const auto names = wave_detail::distributed_sieve_worker_attempt_names_v1(
        chunk.relative_artifact_stem, chunk.chunk_id, 0);
    CHECK(names.has_value());
    const auto pristine_snapshot = capture_wave_root_snapshot(root);

    auto generic = store.claim_private_lease_root();
    (void)require_private_lease_root_claim_ready(generic,
                                                 "mint generic root claim for recovery rejection");
    auto generic_rejected = wave_detail::recover_worker_attempt_private_lease(std::move(generic));
    CHECK(!generic_rejected);
    CHECK(generic_rejected.claim == nullptr);
    require_wave_status(generic_rejected.diagnostic,
                        wave_detail::DistributedSieveWaveStoreStatus::invalid_request,
                        "generic root claim cannot enter attempt recovery");
    CHECK(capture_wave_root_snapshot(root) == pristine_snapshot);

    auto missing = store.open_worker_attempt_private_lease_root(chunk.chunk_id, 0);
    CHECK(!missing);
    CHECK(missing.claim == nullptr);
    require_wave_status(missing.diagnostic,
                        wave_detail::DistributedSieveWaveStoreStatus::namespace_conflict,
                        "open-existing recovery never creates a missing BaseLock");
    CHECK(!entry_exists_no_follow(root / names->base_lock_leaf));
    CHECK(capture_wave_root_snapshot(root) == pristine_snapshot);

    auto created_claim = store.create_worker_attempt_private_lease_root(chunk.chunk_id, 0);
    (void)require_private_lease_root_claim_ready(created_claim, "create P0 for origin split");
    const auto p0_snapshot = capture_wave_root_snapshot(root);
    auto create_rejected =
        wave_detail::recover_worker_attempt_private_lease(std::move(created_claim));
    CHECK(!create_rejected);
    CHECK(create_rejected.claim == nullptr);
    require_wave_status(create_rejected.diagnostic,
                        wave_detail::DistributedSieveWaveStoreStatus::invalid_request,
                        "create-origin P0 cannot enter open-existing recovery");
    CHECK(capture_wave_root_snapshot(root) == p0_snapshot);

    auto opened = store.open_worker_attempt_private_lease_root(chunk.chunk_id, 0);
    auto recovered = wave_detail::recover_worker_attempt_private_lease(std::move(opened));
    (void)require_private_lease_root_claim_ready(recovered,
                                                 "open-origin P0 recovery is idempotent");
    CHECK(capture_wave_root_snapshot(root) == p0_snapshot);

    auto recovered_again = wave_detail::recover_worker_attempt_private_lease(std::move(recovered));
    (void)require_private_lease_root_claim_ready(
        recovered_again, "returned open-origin claim remains recovery-capable at P0");
    CHECK(capture_wave_root_snapshot(root) == p0_snapshot);
    CHECK(relation_base_lock_reports_busy(root / names->base_lock_leaf));

    auto reserved = wave_detail::reserve_worker_attempt_private_lease(std::move(recovered_again));
    CHECK(reserved);
    CHECK(reserved.receipt.has_value());
    require_wave_status(reserved.receipt->revalidate(),
                        wave_detail::DistributedSieveWaveStoreStatus::ready,
                        "idempotently recovered P0 claim may reserve freshly");
}

void test_wave_store_private_lease_recovery_authority_exclusion_and_process_binding() {
    using Boundary = wave_detail::DistributedSievePrivateLeaseReservationBoundary;

    WaveStoreTempDirectory temp;
    const auto root = temp.path() / "private-lease-recovery-authority";
    auto created = wave_detail::DistributedSieveWaveStore::create(root, wave_manifest_draft());
    auto& store = require_wave_ready(created, "create recovery-authority fixture");
    const auto& chunk = store.manifest().chunks.front();
    const std::uint32_t chunk_id = chunk.chunk_id;
    const auto names = wave_detail::distributed_sieve_worker_attempt_names_v1(
        chunk.relative_artifact_stem, chunk_id, 0);
    CHECK(names.has_value());
    const auto target = root / names->base_lock_leaf;
    const Digest digest = store.manifest_digest();

    auto fresh = store.create_worker_attempt_private_lease_root(chunk_id, 0);
    (void)require_private_lease_root_claim_ready(fresh, "create recovery-authority BaseLock");
    const auto p0_snapshot = capture_wave_root_snapshot(root);
    WavePrivateLeaseProtocolStopContext initial_reservation_stop{
        .target = Boundary::ReservedPendingDurable,
    };
    auto initial_reservation = wave_detail::reserve_worker_attempt_private_lease(
        std::move(fresh), wave_detail::DistributedSievePrivateLeaseProtocolTestHooks{
                              .stop_after = stop_at_wave_private_lease_boundary,
                              .context = &initial_reservation_stop,
                          });
    CHECK(!initial_reservation);
    CHECK(!initial_reservation.receipt.has_value());
    CHECK(fresh.claim == nullptr);
    require_wave_status(initial_reservation.diagnostic,
                        wave_detail::DistributedSieveWaveStoreStatus::interrupted,
                        "leave P1 for recovery-authority interruption");
    CHECK(initial_reservation_stop
              .observed[static_cast<std::size_t>(Boundary::ReservedPendingDurable)]);

    auto opened_for_interruption = store.open_worker_attempt_private_lease_root(chunk_id, 0);
    (void)require_private_lease_root_claim_ready(opened_for_interruption,
                                                 "open P1 before recovery-authority interruption");
    WavePrivateLeaseRecoveryStopContext recovery_stop{
        .target = Boundary::PermitAcquired,
    };
    auto interrupted = wave_detail::recover_worker_attempt_private_lease(
        std::move(opened_for_interruption),
    ...
    CHECK(!interrupted);
    CHECK(interrupted.claim == nullptr);
    CHECK(opened_for_interruption.claim == nullptr);
    require_wave_status(interrupted.diagnostic,
                        wave_detail::DistributedSieveWaveStoreStatus::interrupted,
                        "interrupted recovery returns no live authority");
    CHECK(interrupted.diagnostic.last_private_lease_recovery_boundary == Boundary::PermitAcquired);
    CHECK(recovery_stop.observed[static_cast<std::size_t>(Boundary::PermitAcquired)]);
    CHECK(capture_wave_root_snapshot(root) == p0_snapshot);
    CHECK(!relation_base_lock_reports_busy(target));

    auto reopened_after_interruption = store.open_worker_attempt_private_lease_root(chunk_id, 0);
    auto& reopened_after_interruption_claim = require_private_lease_root_claim_ready(
        reopened_after_interruption, "interrupted recovery releases claim slot and target lock");
    require_wave_status(reopened_after_interruption_claim.revalidate(),
                        wave_detail::DistributedSieveWaveStoreStatus::ready,
                        "reopened P0 claim after recovery interruption");
    CHECK(relation_base_lock_reports_busy(target));

    WavePrivateLeaseProtocolStopContext repeated_reservation_stop{
        .target = Boundary::ReservedPendingDurable,
    };
    auto repeated_reservation = wave_detail::reserve_worker_attempt_private_lease(
        std::move(reopened_after_interruption),
        wave_detail::DistributedSievePrivateLeaseProtocolTestHooks{
            .stop_after = stop_at_wave_private_lease_boundary,
            .context = &repeated_reservation_stop,
        });
    CHECK(!repeated_reservation);
    CHECK(!repeated_reservation.receipt.has_value());
    CHECK(reopened_after_interruption.claim == nullptr);
    require_wave_status(repeated_reservation.diagnostic,
                        wave_detail::DistributedSieveWaveStoreStatus::interrupted,
                        "recreate P1 after recovery interruption");

    auto opened = store.open_worker_attempt_private_lease_root(chunk_id, 0);
    auto recovered = wave_detail::recover_worker_attempt_private_lease(std::move(opened));
    auto& recovered_claim =
        require_private_lease_root_claim_ready(recovered, "recover P1 to a retained P0 authority");
    require_wave_status(recovered_claim.revalidate(),
                        wave_detail::DistributedSieveWaveStoreStatus::ready,
                        "returned recovery authority revalidates at P0");
    CHECK(capture_wave_root_snapshot(root) == p0_snapshot);
    CHECK(relation_base_lock_reports_busy(target));

    std::array<std::unique_ptr<PrivateLeaseRootClaim>, 2> contender_claims;
    std::array<wave_detail::DistributedSieveWaveStoreDiagnostic, 2> contender_diagnostics;
    std::barrier<> contention_phases(2);
    std::thread contender([&] {
        contention_phases.arrive_and_wait();
        auto contender_open = store.open_worker_attempt_private_lease_root(chunk_id, 0);
        contender_diagnostics[0] = contender_open.diagnostic;
        contender_claims[0] = std::move(contender_open.claim);
        auto contender_create = store.create_worker_attempt_private_lease_root(chunk_id, 0);
        contender_diagnostics[1] = contender_create.diagnostic;
        contender_claims[1] = std::move(contender_create.claim);
        contention_phases.arrive_and_wait();
    });
    contention_phases.arrive_and_wait();
    contention_phases.arrive_and_wait();
    ...
    CHECK(contender_claims[0] == nullptr);
    CHECK(contender_claims[1] == nullptr);
    require_wave_status(contender_diagnostics[0],
                        wave_detail::DistributedSieveWaveStoreStatus::private_lease_root_busy,
                        "recovered claim excludes concurrent open on the same attempt");
    require_wave_status(contender_diagnostics[1],
                        wave_detail::DistributedSieveWaveStoreStatus::private_lease_root_busy,
                        "recovered claim excludes concurrent create on the same attempt");
    require_wave_status(recovered_claim.revalidate(),
                        wave_detail::DistributedSieveWaveStoreStatus::ready,
                        "recovered authority survives concurrent contenders");
    CHECK(relation_base_lock_reports_busy(target));
    CHECK(capture_wave_root_snapshot(root) == p0_snapshot);
    ...
    if (child == 0) {
        (void)::close(ready_pipe[0]);
        (void)::close(release_pipe[1]);
        const bool inherited_not_owned =
            recovered.claim != nullptr && !recovered.claim->owned_by_current_process();
        auto inherited_recovery =
            wave_detail::recover_worker_attempt_private_lease(std::move(recovered));
        const bool rejected = inherited_not_owned && !inherited_recovery &&
                              inherited_recovery.claim == nullptr && recovered.claim == nullptr &&
                              inherited_recovery.diagnostic.status ==
                                  wave_detail::DistributedSieveWaveStoreStatus::invalid_request;
        const bool signalled = write_pipe_byte(ready_pipe[1], rejected ? 'r' : 'f');
        char release = '\0';
        const bool released = read_pipe_byte(release_pipe[0], release);
        ::_exit(rejected && signalled && released && release == 'x' ? 0 : 86);
    }
    ...
    require_wave_status(recovered_claim.revalidate(),
                        wave_detail::DistributedSieveWaveStoreStatus::ready,
                        "parent recovery authority survives inherited child rejection");
    CHECK(relation_base_lock_reports_busy(target));
    ...
    require_wave_status(recovered_claim.revalidate(),
                        wave_detail::DistributedSieveWaveStoreStatus::ready,
                        "parent recovery authority survives child close and exit");
    CHECK(relation_base_lock_reports_busy(target));

    auto continued = wave_detail::recover_worker_attempt_private_lease(std::move(recovered));
    auto& continued_claim = require_private_lease_root_claim_ready(
        continued, "parent can continue recovery after child rejection");
    CHECK(recovered.claim == nullptr);
    require_wave_status(continued_claim.revalidate(),
                        wave_detail::DistributedSieveWaveStoreStatus::ready,
                        "continued parent recovery retains exact P0 authority");
    CHECK(capture_wave_root_snapshot(root) == p0_snapshot);

    created.store.reset();
    require_wave_status(continued_claim.revalidate(),
                        wave_detail::DistributedSieveWaveStoreStatus::ready,
                        "recovered claim outlives its WaveStore facade");
    auto second_store_while_claimed = wave_detail::DistributedSieveWaveStore::open(root, digest);
    CHECK(!second_store_while_claimed);
    CHECK(second_store_while_claimed.store == nullptr);
    require_wave_status(second_store_while_claimed.diagnostic,
                        wave_detail::DistributedSieveWaveStoreStatus::lock_busy,
                        "recovered claim excludes a second WaveStore");

    continued.claim.reset();
    CHECK(!relation_base_lock_reports_busy(target));
    auto reopened_store_result = wave_detail::DistributedSieveWaveStore::open(root, digest);
    auto& reopened_store =
        require_wave_ready(reopened_store_result, "open WaveStore after recovered claim release");
    auto reopened_attempt = reopened_store.open_worker_attempt_private_lease_root(chunk_id, 0);
    auto reopened_recovery =
        wave_detail::recover_worker_attempt_private_lease(std::move(reopened_attempt));
    auto& reopened_recovery_claim = require_private_lease_root_claim_ready(
        reopened_recovery, "reopen and recover same attempt through a new WaveStore");
    require_wave_status(reopened_recovery_claim.revalidate(),
                        wave_detail::DistributedSieveWaveStoreStatus::ready,
...
void test_wave_store_attempt_record_inventory_accepts_closed_states() {
    {
        WaveStoreTempDirectory temp;
        const auto root = temp.path() / "attempt-record-canonical-chain-p0";
        auto created = wave_detail::DistributedSieveWaveStore::create(root, wave_manifest_draft());
        auto& store = require_wave_ready(created, "create canonical-chain P0 fixture");
        const auto& chunk = store.manifest().chunks.front();
        ...
        create_wave_attempt_p0(store, chunk.chunk_id, 0, "create attempt 0 P0");
        create_wave_attempt_p0(store, chunk.chunk_id, 1, "create attempt 1 P0");
        const auto attempt_0 =
            make_wave_attempt_started(store.manifest(), chunk, 0, store.manifest_digest(),
                                      lease_identity(900, names_0->relative_lease_stem));
        const auto attempt_1 =
            make_wave_attempt_started(store.manifest(), chunk, 1, attempt_0.self_digest,
                                      lease_identity(930, names_1->relative_lease_stem));
        publish_wave_attempt_canonical_record(root, *names_0, attempt_0,
                                              "publish canonical attempt 0");
        publish_wave_attempt_canonical_record(root, *names_1, attempt_1,
                                              "publish canonical attempt 1");

        require_wave_attempt_inventory_reopens(
            created, root, "canonical attempt chain with cleaned P0 leases reopens");
    }

    {
        WaveStoreTempDirectory temp;
        const auto root = temp.path() / "attempt-record-canonical-chain-p8";
        auto created = wave_detail::DistributedSieveWaveStore::create(root, wave_manifest_draft());
        auto& store = require_wave_ready(created, "create canonical-chain P8 fixture");
        const auto& chunk = store.manifest().chunks.front();
        const auto names_0 = wave_detail::distributed_sieve_worker_attempt_names_v1(
            chunk.relative_artifact_stem, chunk.chunk_id, 0);
        CHECK(names_0.has_value());

        create_wave_attempt_p0(store, chunk.chunk_id, 0, "create canonical predecessor P0");
        {
            auto receipt =
                reserve_wave_attempt_p8(store, chunk.chunk_id, 1, "reserve next attempt P8");
            const auto attempt_0 =
                make_wave_attempt_started(store.manifest(), chunk, 0, store.manifest_digest(),
                                          lease_identity(960, names_0->relative_lease_stem));
            publish_wave_attempt_canonical_record(root, *names_0, attempt_0,
                                                  "publish predecessor before next P8");
            CHECK(receipt.owned_by_current_process());
        }

        require_wave_attempt_inventory_reopens(
            created, root, "canonical predecessor plus next-attempt P8 reopens");
    }

    {
        WaveStoreTempDirectory temp;
        const auto root = temp.path() / "attempt-record-pending-next-p8";
        auto created = wave_detail::DistributedSieveWaveStore::create(root, wave_manifest_draft());
        auto& store = require_wave_ready(created, "create next-pending P8 fixture");
        const auto& chunk = store.manifest().chunks.front();
        const auto names_0 = wave_detail::distributed_sieve_worker_attempt_names_v1(
            chunk.relative_artifact_stem, chunk.chunk_id, 0);
        const auto names_1 = wave_detail::distributed_sieve_worker_attempt_names_v1(
        ...
        create_wave_attempt_p0(store, chunk.chunk_id, 0, "create pending predecessor P0");
        {
            auto receipt =
                reserve_wave_attempt_p8(store, chunk.chunk_id, 1, "reserve pending attempt P8");
            const auto attempt_0 =
                make_wave_attempt_started(store.manifest(), chunk, 0, store.manifest_digest(),
                                          lease_identity(990, names_0->relative_lease_stem));
            const auto attempt_1 =
                make_wave_attempt_started(store.manifest(), chunk, 1, attempt_0.self_digest,
                                          wave_attempt_lease_from_receipt(receipt));
            publish_wave_attempt_canonical_record(root, *names_0, attempt_0,
                                                  "publish pending predecessor");
            publish_wave_attempt_fixture_leaf(root, names_1->pending_record_leaf, attempt_1,
                                              "publish exact next-attempt pending leaf");
        ...
        require_wave_attempt_inventory_reopens(created, root,
                                               "pending-only next ordinal with exact P8 reopens");
    }

    {
        WaveStoreTempDirectory temp;
        const auto root = temp.path() / "attempt-record-identical-dual-p8";
        auto created = wave_detail::DistributedSieveWaveStore::create(root, wave_manifest_draft());
        auto& store = require_wave_ready(created, "create identical dual-record P8 fixture");
        const auto& chunk = store.manifest().chunks.front();
        const auto names = wave_detail::distributed_sieve_worker_attempt_names_v1(
            chunk.relative_artifact_stem, chunk.chunk_id, 0);
        CHECK(names.has_value());
        ...
        {
            auto receipt =
                reserve_wave_attempt_p8(store, chunk.chunk_id, 0, "reserve dual-record P8");
            const auto attempt =
                make_wave_attempt_started(store.manifest(), chunk, 0, store.manifest_digest(),
                                          wave_attempt_lease_from_receipt(receipt));
            publish_wave_attempt_canonical_record(root, *names, attempt,
                                                  "publish dual-record canonical");
            publish_wave_attempt_fixture_leaf(root, names->pending_record_leaf, attempt,
                                              "publish identical durable pending duplicate");
            CHECK(read_file_bytes(root / names->canonical_record_leaf) ==
                  read_file_bytes(root / names->pending_record_leaf));
        }

        require_wave_attempt_inventory_reopens(
            created, root, "identical canonical and pending records with exact P8 reopen");
    }
}

void test_wave_store_create_rejects_pre_manifest_attempt_record_leaf() {
    const auto draft = wave_manifest_draft();
    const auto& chunk = draft.chunks.front();
    const auto names = wave_detail::distributed_sieve_worker_attempt_names_v1(
        chunk.relative_artifact_stem, chunk.chunk_id, 0);
    ...
    for (const bool pending : {false, true}) {
        WaveStoreTempDirectory temp;
        const auto root = temp.path() / (pending ? "pre-manifest-pending-attempt-leaf"
                                                 : "pre-manifest-canonical-attempt-leaf");
        std::error_code error;
        CHECK(std::filesystem::create_directory(root, error));
        CHECK(!error);
        require_chmod(root, 0700, "chmod pre-manifest attempt root");
        const auto record_leaf =
            root / (pending ? names->pending_record_leaf : names->canonical_record_leaf);
        write_foreign_leaf(record_leaf);
        const auto before = capture_wave_root_snapshot(root);
        CHECK(before.size() == 2U);
        CHECK(require_wave_root_snapshot_leaf(before, record_leaf.filename().string()).bytes ==
              read_file_bytes(record_leaf));

        WaveFaultStopContext hooks{
            .target = wave_detail::DistributedSieveWaveStoreFaultPoint::ManifestCanonicalDurable,
        };
        auto rejected = wave_detail::DistributedSieveWaveStore::create(
            root, wave_manifest_draft(),
            wave_detail::DistributedSieveWaveStoreTestHooks{
                .stop_after = stop_at_wave_fault,
                .context = &hooks,
        ...
        require_wave_status(
            rejected.diagnostic, wave_detail::DistributedSieveWaveStoreStatus::namespace_conflict,
            pending ? "pre-manifest pending attempt leaf rejects create before lock"
                    : "pre-manifest canonical attempt leaf rejects create before lock");
        CHECK(hooks.observed[static_cast<std::size_t>(
            wave_detail::DistributedSieveWaveStoreFaultPoint::RootDurable)]);
        CHECK(std::ranges::none_of(
            hooks.observed.begin() +
                static_cast<std::ptrdiff_t>(
                    wave_detail::DistributedSieveWaveStoreFaultPoint::LockDurable),
            hooks.observed.end(), [](bool observed) { return observed; }));
        CHECK(capture_wave_root_snapshot(root) == before);
        CHECK(!entry_exists_no_follow(wave_lock_path(root)));
        CHECK(!entry_exists_no_follow(wave_manifest_path(root)));
        CHECK(!entry_exists_no_follow(wave_manifest_pending_path(root)));
