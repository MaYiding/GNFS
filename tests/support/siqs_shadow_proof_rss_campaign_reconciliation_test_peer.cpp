#include "siqs_shadow_proof_rss_campaign_reconciliation_test_peer.hpp"
...
ReconciliationTestPeer::reconcile(const SIQSShadowProofRssGatePolicy* policy,
    ...
    try {
        ...
        if (!absent_journal_preflight_is_ready(claims_preflight)) {
            ...
            return {CampaignReconciliationOutcome::admission_rejected, std::move(diagnostic)};
        }
        ...
        if (!absent_journal_preflight_is_ready(approved_preflight) ||
            ...
            runtime_facts == nullptr || !policy_claim_matches(*policy, binding.approved_policy_) ||
            !runtime_claim_matches(*runtime_facts, binding.approved_runtime_facts_)) {
            return {
                CampaignReconciliationOutcome::admission_rejected,
        ...
        return ReconciliationOrchestrator::reconcile_approved(std::move(binding));
