// test_sieve_norm_tile.cpp - Correctness tests for the row-tile gate
// helper exposed in `<gnfs/sieve/norm_tile.hpp>`.
//
// Strategy
// --------
// The helper only exposes an ENV-cached integer (`norm_tile_bits`) and
// two derived predicates (`norm_tile_enabled`, `norm_tile_size_rows`).
...
// helper is a standalone gate (complementary to but distinct from the
// W6 region_tile gate), and the norm-precompute wiring is a follow-up
// change. We just need to lock the contract down before any callsite
// starts depending on it.

#include <gnfs/sieve/norm_tile.hpp>

#include <cstdio>
#include <cstdlib>
#include <string>

namespace nt = gnfs::sieve;
...
// cached state so a subsequent `norm_tile_bits()` call re-resolves.
static void set_env_and_reload(const char* value) {
    if (value == nullptr) {
        ::unsetenv("GNFS_SIEVE_NORM_TILE_BITS");
    } else {
        ::setenv("GNFS_SIEVE_NORM_TILE_BITS", value, /*overwrite=*/1);
    }
    nt::norm_tile_reset_env_cache_for_testing();
}
...
// branch on a single predicate at precompute-phase entry without an
// additional check.
static void test_env_unset_default_zero() {
    std::printf("[1] env unset defaults to N=0\n");
    set_env_and_reload(nullptr);

    TEST_ASSERT(nt::norm_tile_bits() == 0, "unset bits should be 0");
    TEST_ASSERT(!nt::norm_tile_enabled(),
                "unset must not enable tiling");
    TEST_ASSERT(nt::norm_tile_size_rows() == 0,
...
static void test_env_zero_explicit() {
    ...
    TEST_ASSERT(nt::norm_tile_bits() == 0,
...
static void test_env_valid_range() {
    ...
    for (int n = 1; n <= nt::kNormTileMaxBits; ++n) {
        const std::string s = std::to_string(n);
        set_env_and_reload(s.c_str());

        const int got = nt::norm_tile_bits();
        if (got != n) {
            std::fprintf(stderr,
        ...
        if (!nt::norm_tile_enabled()) {
            std::fprintf(stderr,
                         "  N=%d enabled=false (expected true)\n", n);
            tests_failed++;
            return;
        }
        const std::size_t rows = nt::norm_tile_size_rows();
...
// Test 4 - Values strictly above kNormTileMaxBits clamp to the max so
...
static void test_env_above_max_clamps_to_8() {
    ...
    TEST_ASSERT(nt::norm_tile_bits() == nt::kNormTileMaxBits,
                "N=9 should clamp to kNormTileMaxBits");
    TEST_ASSERT(nt::norm_tile_size_rows() == 256,
                "N=9 clamped tile size must be 256 rows");

    set_env_and_reload("100");
    TEST_ASSERT(nt::norm_tile_bits() == nt::kNormTileMaxBits,
                "N=100 should clamp to kNormTileMaxBits");
    TEST_ASSERT(nt::norm_tile_size_rows() == 256,
...
static void test_norm_tile_size_rows() {
    std::printf("[6] norm_tile_size_rows == 2^N\n");
    const std::size_t expected[] = {
        ...
        64,   // N=6
        128,  // N=7
        256,  // N=8 = kNormTileMaxBits
    };
    for (int n = 0; n <= nt::kNormTileMaxBits; ++n) {
        if (n == 0) {
            set_env_and_reload("0");
        } else {
            const std::string s = std::to_string(n);
        ...
        const std::size_t got = nt::norm_tile_size_rows();
        if (got != expected[n]) {
            std::fprintf(stderr,
    ...
    TEST_PASS("norm_tile_size_rows == 2^N");
}
...
static void test_reset_env_cache() {
    ...
    TEST_ASSERT(nt::norm_tile_bits() == 4,
                "reset to N=4 should yield 4");
    TEST_ASSERT(nt::norm_tile_size_rows() == 16,
    ...
    TEST_ASSERT(nt::norm_tile_bits() == 6,
...
// invariant (callers branch on `norm_tile_enabled()` at precompute-phase
...
int main() {
    std::printf("=== test_sieve_norm_tile ===\n");
    const char* env = std::getenv("GNFS_SIEVE_NORM_TILE_BITS");
    std::printf("GNFS_SIEVE_NORM_TILE_BITS = %s\n", env ? env : "(unset)");
