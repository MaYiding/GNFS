...
FactResult factor_with_progress(const Integer& n, int level) {
        ...
        if (policy.enabled) {
            ...
            if (const char* path_env = std::getenv("GNFS_OOC_BASE_PATH");
                ...
                coll_config.ooc_base_path = gnfs::util::temp_path(
                    "gnfs_progressive_relations_" + std::to_string(gnfs::util::process_id()));
            }
    ...
    for (int round = 0; round < MAX_ROUNDS; ++round) {
        ...
        while (sq_gen.has_next() && collector.size() < batch_target &&
            ...
            if (sq_count % std::max(size_t(1), static_cast<size_t>(params.progress_interval)) ==
                    0 ||
    ...
    // See test_stress.cpp:478 for the same fix.
    {
        ...
        size_t max_rels = util::saturating_size_add(effective_cols_for_trim,
                                                    effective_cols_for_trim / 4); // 25% safety
        if (relations.size() > max_rels) {
            std::cout << "  [Trim] " << relations.size() << " → " << max_rels
                      << " relations (eff_cols=" << effective_cols_for_trim << ", excess "
                      << (relations.size() - max_rels) << " dropped)\n";
    ...
    for (size_t di = 0; di < deps.size(); ++di) {
        ...
        if (!verify_dep(build_result.matrix, dep))
            continue;
        ...
        if (di < 3) {
            std::cout << " X=" << rat.value.to_string() << " Y=" << alg.value.to_string()
                      << std::flush;
        ...
        for (size_t i = 0; i < max_try && !result.success; ++i) {
            for (size_t j = i + 1; j < max_try && !result.success; ++j) {
                ...
                if (combined.popcount() < 2)
                    continue;
                if (!verify_dep(build_result.matrix, combined))
                    continue; // verify combined
    ...
    if (result.success) {
        ...
        std::cout << "│  Time: " << std::fixed << std::setprecision(2) << result.time_sec
                  << " sec\n";
        std::cout << "└──────────────────────────────────────┘\n" << std::flush;
    } else {
