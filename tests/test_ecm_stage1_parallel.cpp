...
void test_n1_sequential_baseline() {
    ...
    std::vector<uint64_t> sigmas = {100, 101, 102, 103, 104, 105, 106, 107, 108, 109};

    auto results = parallel_stage1_curves<uint64_t, uint64_t>(
        std::span<const uint64_t>(sigmas),
        [](uint64_t sigma) -> uint64_t { return sigma * sigma + 1; });

    if (results.size() != sigmas.size()) {
        std::cerr << "\n  ERROR: expected " << sigmas.size() << " results, got " << results.size()
                  << std::endl;
        std::abort();
    }
...
void test_n4_parallel_parity() {
    std::cout << "Test 5: N=1 vs N=4 parity (per-index bit-identical)..." << std::flush;
    ...
    for (uint64_t s = 200; s < 232; ++s)
        sigmas.push_back(s);

    apply_env("1");
    auto seq =
        parallel_stage1_curves<uint64_t, uint64_t>(std::span<const uint64_t>(sigmas), worker);
...
void test_empty_curve_list() {
    std::cout << "Test 6: empty curve list (no-op both paths)..." << std::flush;
    ...
    apply_env("1");
    auto seq_results =
        parallel_stage1_curves<int, int>(std::span<const int>(empty_in), [](int) -> int {
            std::cerr << "\n  ERROR: should not invoke run_stage1 on empty span" << std::endl;
            std::abort();
            return 0;
        });
...
void test_single_curve_no_stall() {
    std::cout << "Test 7: single-curve N=4 (exactly-once, no stall)..." << std::flush;
    ...
    auto results = parallel_stage1_curves<uint64_t, uint64_t>(
        std::span<const uint64_t>(single), [&calls](uint64_t sigma) -> uint64_t {
...
void test_n_hw_parity() {
    ...
    std::cout << " PASS (N=hw=" << hw << ", " << sigmas.size() << " per-index identical)\n";
}
...
void test_real_ecm_stage1_parity() {
    std::cout << "Test 9: real ECM Stage 1 parity (N=1 vs N=4)..." << std::flush;
    ...
    std::cout << " PASS (" << sigmas.size() << " curves, " << any_factor_seq << " found factor)\n";
}
...
int main() {
    ...
    std::cout << std::endl << "=== All ECM Stage 1 Parallel Tests PASSED ===" << std::endl;
    return 0;
}
