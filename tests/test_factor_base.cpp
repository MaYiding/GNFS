...
void test_serialization_invalid() {
    ...
    // A hostile count must be rejected before resize/allocation.
    {
        std::stringstream ss;
        write_valid_prefix(ss, 0, std::numeric_limits<uint32_t>::max());
        expect_load_failure(ss);
    }
    ...
    // The sieve prefix cannot claim more entries than the algebraic side.
    {
        std::stringstream ss;
        write_valid_prefix(ss, 1, 0);
        const uint32_t alg_count = 0;
        write_value(ss, alg_count);
        expect_load_failure(ss);
    }
