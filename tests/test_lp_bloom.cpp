...
//      positives, and membership transitions after insertion.
...
void test_env_unset_is_disabled() {
    ...
    GNFS_TEST_CHECK(filter_lp_bloom_bits() == 0);
    GNFS_TEST_CHECK(!filter_lp_bloom_enabled());
...
void test_env_in_range_value() {
    ...
    GNFS_TEST_CHECK(filter_lp_bloom_bits() == 10);
    GNFS_TEST_CHECK(filter_lp_bloom_enabled());

    set_env_and_reload("14");
    GNFS_TEST_CHECK(filter_lp_bloom_bits() == 14);

    set_env_and_reload("16");
    GNFS_TEST_CHECK(filter_lp_bloom_bits() == 16);
    GNFS_TEST_CHECK(filter_lp_bloom_enabled());

    set_env_and_reload("22");
    GNFS_TEST_CHECK(filter_lp_bloom_bits() == 22);

    set_env_and_reload("28");
    GNFS_TEST_CHECK(filter_lp_bloom_bits() == 28);
...
void test_bloom_basic_insert_and_contains() {
    ...
    BloomLPKeyFilter bloom(14); // 2 KiB filter
    const std::uint64_t key = 0xDEADBEEFCAFEBABEULL;
    GNFS_TEST_CHECK(!bloom.maybe_contains(key));
    bloom.insert(key);
    GNFS_TEST_CHECK(bloom.maybe_contains(key));
    // Distinct key should usually not collide on all 4 hash slots.
    GNFS_TEST_CHECK(!bloom.maybe_contains(key + 1));
    std::cout << "  PASS" << std::endl;
}

void test_bloom_no_false_negatives() {
    std::cout << "Testing Bloom: no false negatives across 10k inserts..." << std::endl;
    BloomLPKeyFilter bloom(18); // 32 KiB filter — ~0.04% FP @ 10k
    ...
    for (std::uint64_t k : keys) {
        GNFS_TEST_CHECK(bloom.maybe_contains(k));
    }
    std::cout << "  PASS" << std::endl;
}

void test_bloom_false_positive_rate_under_5pct() {
    std::cout << "Testing Bloom: FP rate < 5% for bits=20, n=10000..." << std::endl;
    BloomLPKeyFilter bloom(20); // 128 KiB filter
    ...
    std::cout << "    inserted=" << inserted.size() << " queries=" << queries << " fp=" << fp
              << " rate=" << rate << " (theoretical=" << theory << ")" << std::endl;
    GNFS_TEST_CHECK(rate < 0.05);
    std::cout << "  PASS" << std::endl;
}
...
void test_count_unique_parity_random_100k() {
    std::cout << "Testing count_unique_with_bloom parity (100k keys)..." << std::endl;
    ...
    const std::size_t baseline = count_unique_with_bloom(keys.begin(), keys.end(), 0);

    // Sweep across several bit widths.
    for (int bits : {10, 14, 18, 22}) {
        const std::size_t with_bloom = count_unique_with_bloom(keys.begin(), keys.end(), bits);
        if (with_bloom != baseline) {
            std::cerr << "[FAIL] bits=" << bits << " baseline=" << baseline
                      << " with_bloom=" << with_bloom << std::endl;
        }
        GNFS_TEST_CHECK(with_bloom == baseline);
    }
...
void test_edge_empty_input() {
    ...
    GNFS_TEST_CHECK(count_unique_with_bloom(empty.begin(), empty.end(), 0) == 0);
    GNFS_TEST_CHECK(count_unique_with_bloom(empty.begin(), empty.end(), 16) == 0);
    GNFS_TEST_CHECK(count_unique_with_bloom(empty.begin(), empty.end(), 22) == 0);
    std::cout << "  PASS" << std::endl;
}
...
void test_edge_single_key_and_all_same() {
    ...
    GNFS_TEST_CHECK(count_unique_with_bloom(single.begin(), single.end(), 0) == 1);
    GNFS_TEST_CHECK(count_unique_with_bloom(single.begin(), single.end(), 16) == 1);

    // 1000 copies of the same key → unique count is 1, regardless of path.
    std::vector<std::uint64_t> all_same(1000, 0xDEADBEEFULL);
    GNFS_TEST_CHECK(count_unique_with_bloom(all_same.begin(), all_same.end(), 0) == 1);
    GNFS_TEST_CHECK(count_unique_with_bloom(all_same.begin(), all_same.end(), 14) == 1);
    GNFS_TEST_CHECK(count_unique_with_bloom(all_same.begin(), all_same.end(), 22) == 1);
    std::cout << "  PASS" << std::endl;
}
...
void test_ctor_rejects_below_floor_and_above_ceiling() {
    ...
    GNFS_TEST_CHECK(floor.bits() == 10);
    GNFS_TEST_CHECK(floor.size_bytes() == 128); // 2^10 / 8 = 128 bytes

    BloomLPKeyFilter ceiling(30);
    GNFS_TEST_CHECK(ceiling.bits() == 30);
    GNFS_TEST_CHECK(ceiling.size_bytes() == (1u << 30) / 8u);
    std::cout << "  PASS" << std::endl;
}
