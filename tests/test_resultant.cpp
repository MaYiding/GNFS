...
std::vector<std::vector<Integer>> make_matrix(
    ...
    for (const auto& r : rows) {
        ...
        row.reserve(r.size());
        for (int64_t v : r) row.emplace_back(v);
        M.push_back(std::move(row));
    }
    return M;
}

std::vector<Integer> make_poly(const std::vector<int64_t>& coeffs) {
    std::vector<Integer> p;
    p.reserve(coeffs.size());
    for (int64_t c : coeffs) p.emplace_back(c);
    return p;
}
...
#define ASSERT_INT_EQ(actual, expected_int64) \
    do { \
        Integer expected_int{static_cast<int64_t>(expected_int64)}; \
        if (!(actual == expected_int)) { \
            std::cerr << "  FAIL: " << #actual << " = " << to_string(actual) \
                      << " expected " << (expected_int64) << std::endl; \
...
void test_bareiss_identity() {
    ...
    for (uint32_t n : {1u, 2u, 3u, 4u, 5u}) {
        ...
        for (uint32_t i = 0; i < n; ++i) I[i][i] = Integer(1);
...
void test_bareiss_large_values() {
    ...
    // Use values that would overflow int64_t in intermediate products.
    ...
    Integer det = bareiss_determinant(M, 2);
    Integer expected = Integer(int64_t(5000000000ll)) - Integer(int64_t(63));
    if (!(det == expected)) {
        std::cerr << "  large: got=" << to_string(det)
