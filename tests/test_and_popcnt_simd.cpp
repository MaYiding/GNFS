// test_and_popcnt_simd.cpp - Correctness tests for the SIMD-accelerated GF(2)
// batch AND-then-popcount helpers.
//
// Strategy
// --------
...
//
// Additional coverage:
// * ENV parsing (GNFS_GF2_AND_POPCNT_SIMD = 0 / 1 / auto / unset / garbage).
// * Aligned vs unaligned batch sizes — the SIMD path must fall through
//   cleanly to the scalar residual tail.
// * total_and_popcount_words: sum reduction equivalence.
...
#include <gnfs/linalg/detail/and_popcnt_simd.hpp>
...
static void set_env_and_reload(const char* value) {
    if (value == nullptr) {
        ::unsetenv("GNFS_GF2_AND_POPCNT_SIMD");
    } else {
        ::setenv("GNFS_GF2_AND_POPCNT_SIMD", value, 1);
    }
    detail::and_popcnt_simd_reset_env_cache_for_testing();
}
...
static void compare_batch(const std::vector<std::uint64_t>& a,
    ...
    detail::batch_and_popcount_words_scalar(a, b, scalar_out);
    ...
    std::uint64_t scalar_total = detail::total_and_popcount_words_scalar(a, b);
    std::uint64_t dispatch_total = detail::total_and_popcount_words(a, b);
...
static void test_env_unset_auto() {
    ...
    detail::AndPopcntSimdMode mode = detail::and_popcnt_simd_mode();
    TEST_ASSERT(mode == detail::AndPopcntSimdMode::Auto,
                "unset env should resolve to Auto");
    const bool enabled = detail::and_popcnt_simd_enabled();
    const bool supported = detail::and_popcnt_simd_supported();
...
static void test_env_force_off() {
    ...
    detail::AndPopcntSimdMode mode = detail::and_popcnt_simd_mode();
    TEST_ASSERT(mode == detail::AndPopcntSimdMode::ForceOff,
                "env '0' should resolve to ForceOff");
    TEST_ASSERT(!detail::and_popcnt_simd_enabled(),
...
static void test_env_force_on() {
    ...
    detail::AndPopcntSimdMode mode = detail::and_popcnt_simd_mode();
    TEST_ASSERT(mode == detail::AndPopcntSimdMode::ForceOn,
...
static void test_env_garbage_fallback() {
    ...
    for (const char* value : {"auto", "", "garbage", "2", "true", "00", "01"}) {
        ...
        detail::AndPopcntSimdMode mode = detail::and_popcnt_simd_mode();
        if (mode != detail::AndPopcntSimdMode::Auto) {
            std::fprintf(stderr,
...
static void test_empty_input() {
    ...
    detail::batch_and_popcount_words(a, b, out);
    TEST_ASSERT(out.empty(),
                "batch_and_popcount_words on empty input must leave out empty");
    std::uint64_t total = detail::total_and_popcount_words(a, b);
    TEST_ASSERT(total == 0,
                "total_and_popcount_words on empty input must be zero");
    // Also exercise scalar reference on empty input.
    detail::batch_and_popcount_words_scalar(a, b, out);
    TEST_ASSERT(out.empty(),
                "scalar reference on empty input must leave out empty");
    TEST_ASSERT(detail::total_and_popcount_words_scalar(a, b) == 0,
                "scalar total on empty input must be zero");
    TEST_PASS("empty input");
}

// Test 6 — single word AND-popcount across hand-verified bit patterns.
static void test_single_word_patterns() {
    ...
    Case cases[] = {
        // Both zero -> 0
        {0ULL, 0ULL, 0},
        // a=0xFF, b=0xFF -> 8 set bits in low byte
        {0xFFULL, 0xFFULL, 8},
        // Disjoint bit patterns: 0xAA & 0x55 = 0
        {0xAAAA'AAAA'AAAA'AAAAULL, 0x5555'5555'5555'5555ULL, 0},
        // Identical 32-bit mask: 0xAA & 0xAA = 0xAA -> 32 set bits
        {0xAAAA'AAAA'AAAA'AAAAULL, 0xAAAA'AAAA'AAAA'AAAAULL, 32},
        // All-ones AND all-ones = all-ones -> 64
        {0xFFFF'FFFF'FFFF'FFFFULL, 0xFFFF'FFFF'FFFF'FFFFULL, 64},
        // a=all-ones, b=0xAA pattern -> 32 set bits
        {0xFFFF'FFFF'FFFF'FFFFULL, 0xAAAA'AAAA'AAAA'AAAAULL, 32},
        // High-bit + low-bit patterns: 0x8000...01 & 0xC000...03 -> popcount 2
        {0x8000'0000'0000'0001ULL, 0xC000'0000'0000'0003ULL, 2},
        // Sparse overlap: 0x0F0F & 0xF0F0 = 0
        {0x0F0F'0F0F'0F0F'0F0FULL, 0xF0F0'F0F0'F0F0'F0F0ULL, 0},
    };
...
// Test 10 — total AND-popcount equivalence across a battery of sizes.
static void test_total_and_popcount_equivalence() {
    std::printf("[10] total AND-popcount equivalence\n");
    set_env_and_reload(nullptr);
    ...
    for (std::size_t n : {std::size_t{1}, std::size_t{2}, std::size_t{3},
        ...
        detail::batch_and_popcount_words(a, b, per_word);
    ...
    TEST_PASS("total AND-popcount equivalence");
}
...
static void test_force_off_vs_auto_parity() {
    ...
    detail::batch_and_popcount_words(a, b, auto_out);
    std::uint64_t auto_total = detail::total_and_popcount_words(a, b);

    // Force-off path.
    set_env_and_reload("0");
    std::vector<std::uint32_t> off_out(1000, 0);
    detail::batch_and_popcount_words(a, b, off_out);
    std::uint64_t off_total = detail::total_and_popcount_words(a, b);
...
static void test_perf_info_1m() {
    ...
    std::uint64_t scalar_total = detail::total_and_popcount_words(a, b);
    ...
    std::uint64_t simd_total = detail::total_and_popcount_words(a, b);
...
static void test_undersized_out_span() {
    ...
    std::vector<std::uint64_t> b = {0xFFULL, 0xAAULL, 0x55ULL, 0xFFFF'FFFF'FFFF'FFFFULL};
    std::vector<std::uint32_t> small_out(2, 0);
    detail::batch_and_popcount_words(a, b, small_out);
    // popcount(0xFF & 0xFF) = 8, popcount(0xAA & 0xAA) = 4.
    TEST_ASSERT(small_out[0] == 8 && small_out[1] == 4,
                "undersized out must contain the first 2 AND-popcounts");
    set_env_and_reload(nullptr);
    TEST_PASS("undersized out span clamping");
}

// Test 14 — same-size input parity: a == b means popcount(a & a) =
// popcount(a). Cross-validates against a plain popcount reference,
// catching any accidental swap of AND for OR / XOR.
static void test_same_input_equivalence_to_popcount() {
    std::printf("[14] a==b equivalence to plain popcount\n");
    set_env_and_reload(nullptr);
...
int main() {
    std::printf("=== test_and_popcnt_simd ===\n");
    std::printf("compile-time SIMD supported: %s\n",
                detail::and_popcnt_simd_supported() ? "yes" : "no");
    const char* env = std::getenv("GNFS_GF2_AND_POPCNT_SIMD");
    std::printf("GNFS_GF2_AND_POPCNT_SIMD = %s\n", env ? env : "(unset)");
    ...
    test_total_and_popcount_equivalence();
    test_force_off_vs_auto_parity();
    test_perf_info_1m();
    test_undersized_out_span();
    test_same_input_equivalence_to_popcount();
