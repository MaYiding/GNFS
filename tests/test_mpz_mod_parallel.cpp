...
//
//   * ENV parsing handles unset / "0" / "1" / "4" / "garbage" / "" / "12abc" /
//     leading whitespace / "10000" correctly; clamping at
...
//   * Empty dividends return cleanly under sequential and parallel settings.
//   * Single dividend under N>=2 returns the correct result without stalling.
...
void test_env_non_numeric() {
    ...
    apply_env("");
    GNFS_TEST_CHECK(mpz_mod_batch_threads() == 1);
...
// ---------------------------------------------------------------------------
// Test 6: Empty dividends return under sequential and parallel settings.
// ---------------------------------------------------------------------------
void test_empty_dividends() {
    std::cout << "Test 6: empty dividends return cleanly..." << std::flush;
    ...
    GNFS_TEST_CHECK(resolve_mpz_mod_batch_threads(0) == 0);
...
void test_single_dividend_n1() {
    ...
    GNFS_TEST_CHECK(results[0].to_uint64() == 243);
...
// ---------------------------------------------------------------------------
// Test 8: Single dividend at N=4 - no stall, correct result.
// ---------------------------------------------------------------------------
void test_single_dividend_n4_no_stall() {
    std::cout << "Test 8: single dividend N=4 (no stall, correct)..." << std::flush;
    apply_env("4");
    ...
    if (ms > 1000) {
        std::cerr << "\n  WARN: single-dividend dispatch took " << ms << " ms (expected << 1000 ms)"
                  << std::endl;
        // No abort -- soft signal, sanitizers can be slow.
    }

    apply_env(nullptr);
    std::cout << " PASS (1000 mod 257 = " << results[0].to_string() << ", " << ms << " ms)\n";
}
...
void test_n1_baseline_matches_scalar() {
    std::cout << "Test 9: N=1 baseline matches scalar mpz_mod..." << std::flush;
    apply_env("1");
...
void test_n1_vs_n4_parity() {
    ...
    std::cout << " PASS (" << dividends.size() << " per-index identical)\n";
}
...
void test_n1_vs_n_hw_parity() {
    ...
    std::cout << " PASS (N=hw=" << hw << ", " << dividends.size() << " per-index identical)\n";
}
...
void test_200bit_modulus_parity() {
    ...
    for (std::size_t i = 0; i < dividends.size(); ++i) {
        ...
        if (mpz_sgn(seq[i].get_mpz()) < 0 || mpz_cmp(seq[i].get_mpz(), modulus.get_mpz()) >= 0) {
            std::cerr << "\n  ERROR: seq idx " << i << " not in [0, modulus)" << std::endl;
            std::abort();
        }
    }

    std::cout << " PASS (" << dividends.size() << " dividends, 200-bit prime modulus)\n";
}
...
void test_dividend_smaller_than_modulus() {
    std::cout << "Test 13: dividend < modulus (residue == dividend)..." << std::flush;
    apply_env("1");
...
void test_dividend_equals_modulus() {
    std::cout << "Test 14: dividend == modulus (residue == 0)..." << std::flush;
    apply_env("1");
    ...
    for (std::size_t i = 0; i < dividends.size(); ++i) {
        if (mpz_sgn(results[i].get_mpz()) != 0) {
            std::cerr << "\n  ERROR: idx " << i << " got " << results[i].to_string()
                      << " expected 0" << std::endl;
            std::abort();
        }
    }
...
void test_negative_dividend_semantics() {
    ...
    for (std::size_t i = 0; i < dividends.size(); ++i) {
        ...
        if (mpz_sgn(results[i].get_mpz()) < 0) {
            std::cerr << "\n  ERROR: idx " << i << " produced negative residue "
...
void test_reset_env_cache_hook() {
    ...
    if (mpz_mod_batch_threads() != expect) {
        std::cerr << "\n  ERROR: post-reset value " << mpz_mod_batch_threads() << " expected "
                  << expect << std::endl;
        std::abort();
    }
...
void test_perf_info_100_dividends() {
    std::cout << "Test 17: perf info (100 dividends, 200-bit modulus)..." << std::flush;
