...
bool test_cjk_width() {
    ...
    assert(display_width("\xe5\xa4\x9a\xe9\xa1\xb9\xe5\xbc\x8f\xe9\x80\x89\xe6\x8b\xa9") ==
           10);                                             // 多项式选择 = 5 chars * 2
    assert(display_width("\xe7\xad\x9b\xe6\xb3\x95") == 4); // 筛法 = 2 chars * 2
    assert(display_width("\xe5\x88\x86\xe8\xa7\xa3\xe6\x88\x90\xe5\x8a\x9f") ==
           8); // 分解成功 = 4 chars * 2
    return true;
}
...
bool test_chinese_strings() {
    ...
    assert(std::strcmp(success, "\xe5\x88\x86\xe8\xa7\xa3\xe6\x88\x90\xe5\x8a\x9f") ==
           0); // 分解成功
    return true;
}
