// test_mpz_mul_parallel.cpp -- batched mpz_mul parallel dispatcher tests
//
...
using gnfs::util::mpz_mul_batch_threads;
using gnfs::util::mpz_mul_batch_threads_reset_env_cache_for_testing;
using gnfs::util::parallel_mpz_mul;
using gnfs::util::resolve_mpz_mul_batch_threads;

namespace {

// Helper: set or unset GNFS_MPZ_MUL_BATCH_THREADS and refresh the cache so
// the next call to mpz_mul_batch_threads() reflects the new value.
void apply_env(const char* value) {
    if (value == nullptr) {
        unsetenv("GNFS_MPZ_MUL_BATCH_THREADS");
    } else {
        setenv("GNFS_MPZ_MUL_BATCH_THREADS", value, /*overwrite=*/1);
    }
    mpz_mul_batch_threads_reset_env_cache_for_testing();
}

// Scalar reference: compute a * b for one pair, returning a fresh Integer.
// Preserves sign per `mpz_mul` standard semantics (positive iff both
// operands' signs agree, zero iff either operand is zero).
Integer scalar_mul(const Integer& a, const Integer& b) {
    Integer out;
    mpz_mul(out.get_mpz(), a.get_mpz(), b.get_mpz());
    return out;
}

// Build a vector of `n` Integer values from a deterministic mt19937_64
// seed, each value uniformly drawn from [0, 2^32) so multi-limb products
// stay within manageable bounds for verification while still exercising
// non-trivial multi-precision multiplication paths.
...
// -> partial-parse rules match W11 T3 / W12 T3 / W13 T5 / W14 T5 family
// semantics (std::atoi accepts a leading numeric prefix; leading whitespace
...
void test_env_non_numeric() {
    ...
    // family (W11 T3 / W12 T3 / W13 T5 / W14 T5). This is intentional
    // family behaviour, documented in the helper header.
    apply_env("  4");
    {
        unsigned int hw = std::thread::hardware_concurrency();
        if (hw == 0) hw = 4;
    ...
    // (matches W11 T3 / W12 T3 / W13 T5 / W14 T5 / W10 T4 / W7-W14 family
    // semantics).
    apply_env("12abc");
    {
        unsigned int hw = std::thread::hardware_concurrency();
...
// ---------------------------------------------------------------------------
// Test 7: Single pair at N=1 -> correct product.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Test 9: N=1 baseline matches scalar mpz_mul reference (5 hand-picked
// cases). Covers the GMP boundary semantics: 0 * 0 = 0, a * 0 = 0,
...
void test_n1_baseline_matches_scalar() {
    ...
    // Specifically verify Case 5's negative sign propagated correctly --
    // this is the main contrast with mpz_gcd which always returns
    // non-negative. mul preserves sign per GMP semantics.
    if (mpz_sgn(results[4].get_mpz()) >= 0) {
        std::cerr << "\n  ERROR: -30 * 12 should produce negative result, got "
                  << results[4].to_string() << std::endl;
        std::abort();
    }

    apply_env(nullptr);
    std::cout << " PASS (5 boundary cases incl. negative sign)\n";
}
...
// ---------------------------------------------------------------------------
// Test 12: 100-bit prime * 100-bit prime pattern: exercises multi-limb
// mpz_mul semantics on a well-defined structural pattern -- not just
...
void test_composite_prime_pattern() {
    ...
    // Product P*Q is approximately 200-208 bits, exercising multi-limb mul.
    std::vector<Integer> a_values;
    std::vector<Integer> b_values;
    ...
    for (std::size_t i = 0; i < primes.size(); ++i) {
        ...
        // Construct P*Q via direct mpz_mul as the reference.
        Integer PQ;
        mpz_mul(PQ.get_mpz(), P.get_mpz(), Q.get_mpz());
        expected.push_back(PQ);
        a_values.push_back(std::move(P));
        b_values.push_back(std::move(Q));
    }
    ...
    std::cout << " PASS (5 pairs, 100-bit primes, multi-limb mul)\n";
}
...
void test_results_undersized_clamp() {
    ...
    // First 3 slots should be ((i+1)*12) * ((i+1)*18) = (i+1)^2 * 216.
...
int main() {
    ...
    test_perf_info_1000_pairs();

    std::cout << "\n=== All MPZ Mul Parallel Tests PASSED ===\n";
    return 0;
}
