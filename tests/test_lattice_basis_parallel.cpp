...
void test_empty_input() {
    ...
    auto seq_results =
        parallel_lattice_basis_reduce<MockReduced, MockBasis>(
            std::span<const MockBasis>(empty_in),
            [](const MockBasis&) -> MockReduced {
                std::cerr << "\n  ERROR: reduce_fn invoked on empty span"
                          << std::endl;
                std::abort();
                return MockReduced{};
            });
...
void test_single_basis_n4_no_stall() {
    std::cout << "Test 7: single basis N=4 (exactly-once, no stall)..."
              << std::flush;
    ...
    auto results = parallel_lattice_basis_reduce<MockReduced, MockBasis>(
        ...
        [&calls](const MockBasis& b) -> MockReduced {
    ...
    if (ms > 1000) {
        std::cerr << "\n  WARN: single-basis dispatch took " << ms
...
void test_100_bases_n1_baseline() {
    std::cout << "Test 8: 100 bases N=1 baseline (identity reduce)..."
              << std::flush;
    apply_env("1");
    ...
    for (std::size_t i = 0; i < bases.size(); ++i) {
        ...
        if (!(results[i] == expect)) {
            std::cerr << "\n  ERROR: idx " << i << " got " << results[i].value
                      << " expected " << expect.value << std::endl;
            std::abort();
        }
    }
...
void test_n1_vs_n4_parity() {
    std::cout << "Test 9: N=1 vs N=4 parity (per-index bit-identical)..."
              << std::flush;
...
void test_n1_vs_n_hw_parity() {
    ...
    double speedup = (ms_par > 0)
                         ? (static_cast<double>(ms_seq) /
                            static_cast<double>(ms_par))
                         : 0.0;
    std::cout << " PASS (N=hw=" << hw << ", " << bases.size()
              << " per-index identical, seq=" << ms_seq << "us, par="
              << ms_par << "us, speedup=" << speedup << "x)\n";
}
...
void test_non_trivial_result_type() {
    std::cout << "Test 11: non-trivial Result (vector<uint64_t>) parity..."
              << std::flush;
    ...
    apply_env("1");
    auto seq =
        parallel_lattice_basis_reduce<std::vector<uint64_t>, MockBasis>(
    ...
    for (std::size_t i = 0; i < seq.size(); ++i) {
        ...
        if (seq[i] != expect) {
            std::cerr << "\n  ERROR: idx " << i
                      << " seq does not match vec_reduce reference"
                      << std::endl;
            std::abort();
        }
    }
...
int main() {
    std::cout << "=== Lattice Basis Parallel Dispatch Tests ==="
              << std::endl;
