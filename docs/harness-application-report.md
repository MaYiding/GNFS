...
> **以 GNFS 项目为例：如何让 AI Agent 成为有纪律的软件工程师**

---

## 引言：核心挑战

当 AI Agent 从"对话助手"升级为"软件工程师"时，我们面临一个根本性问题：**AI 拥有极强的编码能力，但缺乏工程纪律。**

这不是假设性的担忧。在一个工业级 C++20 数论分解项目的实际开发过程中，我们观察到了六个反复出现的失败模式：

**失败模式一：半途而废。** AI 在上下文窗口耗尽或对话自然结束时丢失全部进度，新会话从零开始。一个涉及 50-digit 整数分解的调试会话可能持续数小时，产生的分析结果在会话结束后完全蒸发。

**失败模式二：虚假完成。** AI 在没有运行测试的情况下声称"应该没问题"、"代码已修复"。数论代码中的符号约定错误（`a - b*alpha` vs `a + b*alpha`）在代码审查中极难发现，只有端到端测试才能暴露。

**失败模式三：知识蒸发。** AI 在调试中发现"lp_bits=23 时 LP cols 占 usable 的 64%而非 5%"这类关键知识，随着会话结束而消失。下次遇到同样的 50-digit 矩阵 NO_EXCESS 问题时，AI 会重新花数小时走完同样的调查路径。

**失败模式四：串行低效。** 项目有 62 个相互正交的 ENV 调优开关需要实现。AI 逐个串行实现时，任何一个开关的编译失败都阻塞后续所有工作。

**失败模式五：测试混乱。** 80+ 个测试二进制，耗时从 0.1 秒到 12 小时不等。AI 不知道该跑哪些测试——跑太少遗漏回归，跑太多浪费时间。
...
每个机制按统一结构呈现：**解决的问题 → 采用的方案 → 核心代码示例 → 实际成效。**
...
**算法深度。** 完整的 GNFS 流水线包含 8 个阶段（多项式选择、因子基构建、筛法、余因子分解、关系收集、线性代数、平方根、GCD），每个阶段涉及深层数论知识。AI 如果不了解"元素表示为 a - b*alpha 而非 a + b*alpha"这种约定，就会在符号上犯隐蔽的错误。
...
此外，项目还建立了一套**长周期自主运行规范**（§10），确保 AI 在面对耗时数小时的压力测试时能保持高效运转而不产生进程泄漏或上下文膨胀。
...
**方案：** CLAUDE.md 文件（995 行）作为 AI 的"操作手册"，在每次会话开始时由 Claude Code 自动加载。
...
这个判据在 2026 年 5 月经历了一次大规模验证：原本 5402 行的 CLAUDE.md 被"瘦身重构"为 995 行（减少 82%），62 个 ENV 调优开关的详细设计文档（4620 行）外移到 `docs/env-flags/` 目录。

### 3.2 内容层次

CLAUDE.md 的内容按查阅频率从高到低排列，形成六层结构：
...
├── api/           (6)  # Config, Pipeline, Factorizer, i18n
├── core/          (6)  # Integer, Polynomial, Relation, Params, Types
├── polynomial/    (7)  # Kleinjung 选择, Murphy E, base-m
├── sieve/         (5)  # Lattice sieve, Special-Q, Lattice basis
├── linalg/        (9)  # GF(2) 矩阵, Block Lanczos, Block Wiedemann
├── sqrt/          (7)  # 代数平方根 (Nguyen Hybrid + Couveignes)
└── util/          (7)  # SmallVector, ThreadPool, Logger, Timer
```

**第三层：关键约定与铁律（~120 行）。** 这是 CLAUDE.md 最有价值的部分——从真实 Bug 中提炼的反直觉规则。
...
**示例一：Trim limit 铁律。** 这条铁律来自一个 P1 级 Bug——在 50-digit 分解中，仅按 `matrix_cols` 裁剪关系导致矩阵行数不足（NO_EXCESS）：

```markdown
## Trim limit 必须含 LP cols (P1 BUG 模式, 防 50d/60d NO_EXCESS)

**所有 Phase 4 relation trim 必须使用
`effective_cols = matrix_cols + count_unique_lp_keys(relations)`,**
而非裸 `matrix_cols`。
...
注意这段文字的精确性：它给出了正确公式、列出了 5 个受影响的入口点、指明了触发条件（lp_bits 大于等于 20）。AI 读到这段后，不可能再犯同样的错误。

**示例二：跨规模验证铁律。** 数论算法的一个反直觉特性是小规模测试通过不代表大规模能通过：
...
**示例三：跨平台编译铁律。** 来自 macOS 本地编译通过但 Linux CI 反复失败的教训：
...
**方案：** 六个非 Git 追踪的持久化文件（通过 `.gitignore` 排除），构成跨会话记忆网络。
...
**task_plan.md** 定义"要做什么"。**progress.md** 记录"做到了哪里"，包含每个子阶段的实测数据。**findings.md** 记录"发现了什么"，尤其是计划外的发现。当 AI 在工作中发现了一个计划外的问题时，写入 **BACKLOG.md**。当 BACKLOG 中的问题被修复后，从 BACKLOG 移到 **RESOLVED.md**，包含完整的修复 timeline 和 commit hash。**TODO.md** 独立于这个网络，由用户直接控制。

### 4.3 核心示例：progress.md 的粒度

progress.md 记录的不是"完成了什么功能"，而是每个子阶段的详细执行数据。新会话读取后可以在数分钟内恢复进度：
...
这是精妙的设计决策。这些文件是"会话级持久化"——对当前开发有价值，但不应成为项目历史的一部分。如果入 Git，每次 AI 更新 progress.md 都会产生无意义的 commit，污染 Git 历史。`.gitignore` 中的配置：

```gitignore
# ===== Planning & Tracking Files (Session-Specific) =====
task_plan.md
findings.md
progress.md
BACKLOG.md
...
> 以下机制不属于八大核心机制，但在实际开发中极为重要。它确保 AI 在面对耗时数小时的压力测试时能保持高效运转。

---

## 十一、后台任务与上下文管理

**解决的问题：** GNFS 的压力测试（50-digit/60-digit 分解）可能耗时数小时甚至数十小时。AI 在等待时要么无所事事（浪费时间），要么启动大量监控进程（进程泄漏），要么自行压缩上下文（丢失进度）。

**方案：** CLAUDE.md 定义的后台任务管理规范 + 上下文管理纪律。
...
```bash
# 用 nohup + 文件日志，不用 run_in_background
cd build && nohup ./test_stress 1 1 > /tmp/stress_50d.log 2>&1 &
echo "PID=$! LOG=/tmp/stress_50d.log" >> /tmp/bg_tasks.txt
```

代码中必须加 `std::flush`（nohup 重定向到文件时 C++ `std::cout` 是全缓冲）。

**监控规则（严格）：**

```
禁止堆叠多个 sleep N && tail 后台任务 — 造成进程泄漏
禁止 run_in_background 启动 sleep 循环 — 无法可靠取消
...
CLAUDE.md 明确要求 AI 不要自己管理上下文：

```markdown
## 上下文窗口管理
不要自己压缩上下文。系统会自动处理上下文压缩。

禁止行为:
- 禁止因为担心上下文膨胀而停止工作
- 禁止在工作流中间插入 /compact 导致中断
- 禁止将上下文管理作为暂停或停止的理由
```

这条纪律的价值在于：AI 常常会在上下文窗口接近限制时"恐慌"，主动停止工作或压缩上下文，导致任务中断。这条规则将上下文管理的职责从 AI 身上移除，交给系统自动处理。

### 11.4 成效

AI 在启动 50-digit 压力测试后，立即继续其他优化工作。需要结果时按需 `tail -5 /tmp/xxx.log` 查看日志。不再出现进程泄漏或上下文膨胀问题。
...
| 维度 | 数据 | 说明 |
|------|------|------|
| 总 commit 数 | 2051 | 极细粒度提交 |
| 分支数 | 304 | 保留不删除 |
| Agent Worktree | 51 | 自动管理，全部 locked |
| 手动 Worktree | 20 | 大型特性开发 |
| ENV 调优开关 | 62 | 全部 bit-for-bit 一致 |
| 测试二进制 | 80+ | 跨 5 个时间级别 |
| CLAUDE.md | 995 行 | 瘦身 82% 后的精华 |
| 详细设计文档 | 4620 + 3604 行 | 按需查阅 |
| 持久化文件 | 3170 行 | 6 个文件闭环 |
| 自动化脚本 | 2228 + 295 + 8 个 | 测试 + 分支 + 调优 |
| CI 流水线 | 3 路 | 4 平台 + Sanitizers |
| Superpowers 技能 | 14 个 | 全生命周期覆盖 |
| 自定义 Agent | 2 个 | debugger + reviewer |
| Hook | 3 类 + 权限白名单 | 四层纵深防御 |

### 12.2 效率指标

**并行度。** 51 个 Agent Worktree 在两天内完成了 40+ 个 ENV 开关的并行实现。串行模式下同样的工作量预计需要数周。

**测试效率。** 五级分级使 AI 能精确选择测试级别：日常跑 smoke（5 秒/次），PR 前跑 gate（9 秒 Release 模式），而非每次都跑完整套件（数小时）。

**知识沉淀。** BACKLOG.md 的 11 个问题中 8 个已关闭（含完整修复 timeline），RESOLVED.md 的 1414 行审计记录防止重复修复。

### 12.3 质量指标

**铁律覆盖率。** CLAUDE.md 中的铁律覆盖了所有已知的"AI 容易犯的错误"——跨规模验证、跨平台编译、Trim limit LP cols、关系过滤等。

**Hook 覆盖率。** 从命令执行前（权限白名单）到文件编辑前（PreToolUse）到编辑后（PostToolUse）到对话结束（Stop），四个关键节点全部有自动检查。

**CI 覆盖率。** 四平台构建 + 三路 Sanitizer，捕获了多个"本地 macOS 通过但 Linux CI 失败"的跨平台问题。

---

## 十三、可复用的 Harness 模式

从 GNFS 项目中提炼出八个可复用的模式，按重要性排序：
...
**机制：** task_plan.md、progress.md、BACKLOG.md 等会话级文件通过 .gitignore 排除，允许自由更新而不污染 Git 历史。

**关键设计：** 六个文件有明确的职责边界，形成闭环知识管理网络。

**适用条件：** 任何需要跨会话保持知识连续性的项目。
