...
| Distributed namespace/plan | `GNFS_DISTRIBUTED_SIEVE_WORKERS`, `GNFS_DISTRIBUTED_SIEVE_BASE_PATH`, `GNFS_DISTRIBUTED_SIEVE_SQ_PER_WORKER`, `GNFS_DISTRIBUTED_SIEVE_WORKER_TIMEOUT_MS`; parsed into config, with output-affecting fields bound by the manifest and the root bound by native identity |
