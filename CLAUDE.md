...
> 记录发现但当前不处理的问题。每个条目必须有：分类、状态、来源、描述、发现日期。
...
## 进行中
<!-- 正在修复的条目移到这里，附上开始日期和负责的分支/会话 -->

## 已完成
<!-- 确认解决的条目移到这里，附上解决日期、commit hash 和验证方式 -->
```
...
每个条目必须有明确的状态标记，反映其当前生命周期阶段：

| 状态 | 标记 | 含义 |
|------|------|------|
| 待处理 | 🔴 待处理 | 已记录，尚未开始修复 |
| 进行中 | 🟡 进行中 | 正在当前会话/分支中修复 |
| 待验证 | 🔵 待验证 | 代码已改，等待测试确认 |
| 已完成 | ✅ 已完成 | 修复已验证通过，可关闭 |
...
当开始修复某个 BACKLOG 条目时，**必须同步更新条目状态**，确保进度可追溯：

1. **开始修复时**：将条目状态改为 `🟡 进行中`，移至「进行中」区域，注明开始日期和所在分支
2. **代码修改完成时**：将状态改为 `🔵 待验证`，注明修改了哪些文件、改动摘要
3. **测试验证通过后**：将状态改为 `✅ 已完成`，移至「已完成」区域（详见下方关闭流程）
4. **修复失败/搁置时**：将条目移回「待处理」，状态恢复为 `🔴 待处理`，并追加说明失败原因或搁置理由
...
- **状态**: 🟡 进行中
- **发现日期**: 2026-03-08
- **开始修复**: 2026-03-09，分支 `fix/260309-hensel-overflow`
- **来源**: 在做 sqrt 模块测试时发现
- **描述**: Hensel lifting 中 p^(d-2) 使用 uint64_t，大 d 时溢出
- **改动记录**:
  - `sqrt/hensel_sqrt.hpp:123` — 改用 Integer 类型
```

##### 关闭条目（确认解决）

条目只有满足以下**全部条件**才能标记为已完成并移入「已完成」区域：
...
- **禁止删除未解决条目**：还没修的条目不允许删除，只能保留在「待处理」中
