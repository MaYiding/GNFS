...
- **`BACKLOG.md`**：**待办备忘录**（见下方详细说明），**纳入 git 追踪**

**任务必须自主拆分**为多个阶段。每个小步骤完成后即 commit，不等待整个阶段完成。

### 5.1 BACKLOG.md — 待办备忘录（强制维护）

根目录下维护 `BACKLOG.md`，用于记录**当前会话发现但不处理的事项**。这是跨会话的持久化文件，纳入 git 追踪。

#### 文件结构与排序规则

BACKLOG 已按**严重程度从高到低**严格排序，结构如下：

```
P0 — 关键（阻塞正确性或导致崩溃）
P1 — 高优先（影响可靠性、线程安全、已知功能缺陷）
P1-OPT — 高优先性能优化
P2 — 中优先（边界情况、代码质量、非核心路径问题）
P3 — 低优先（理论风险、技术债务、死代码）
TEST — 测试覆盖缺口
误报 — 经源码验证确认不存在的问题（保留作为审计记录）
已完成 — 已修复并验证通过的条目
```

**查找最重要问题时从文件开头往下读即可**，P0 最先出现，P3 最后。

#### 使用策略

##### 查找待修问题

- **从顶部开始**：P0 条目是最紧急的，优先处理
- **注意跨条目关联**：多个 bug 可能源于同一模块的同一根因。修复一个 bug 前，**先扫描同模块的其他条目**，看是否可以一并解决
  - 例：`sqrt/` 模块可能同时有 Hensel 溢出和 Couveignes 验证两个问题，修 Hensel 时顺便检查 Couveignes
  - 例：`linalg/` 的 SparseRow const_cast 和 multiply_blocks 死代码可能在同一次重构中一起清理
- **简要检查即可**：不需要逐条深度分析，快速扫一遍相关模块的条目标题，判断是否有关联

##### 新增条目

新条目**必须插入到对应严重等级区域的末尾**，保持整体排序不变：

1. 判断严重等级（P0/P1/P2/P3/TEST）
2. 找到该等级区域的最后一个条目
3. 在其后追加新条目
4. 每个条目必须包含：分类标签、严重等级、来源、描述、影响、发现日期

**新条目格式**：
```markdown
### [BUG] 简短标题
- **严重等级**: P1
- **模块**: `sqrt/hensel_sqrt.hpp`
- **发现日期**: 2026-03-09
- **来源**: 在做 XX 任务时发现
- **描述**: 具体问题描述
- **影响**: 影响范围
...
**移入已完成区域时**：
- 在条目标题后加 `✅`，标题加删除线：`### [BUG] ~~标题~~ ✅`
- 追加：解决日期、解决方式（改了哪些文件）、验证命令、Commit hash
- 如果修复同时解决了多个条目，每个条目都要单独标记并说明
...
- **发现日期**: 2026-03-08
- **解决日期**: 2026-03-09
- **解决方式**: `sqrt/hensel_sqrt.hpp:123` 改用 Integer 类型
...
经源码验证确认**不是真实问题**的条目，移入「误报」区域而非删除：
- 说明为什么是误报（附源码证据）
- 保留作为审计记录，避免后续重复上报
...
- **发现即记录**：不要等到阶段结束才补记，发现时立即写入并 commit
- **开始新任务时必读**：每次启动新任务前，先读 `BACKLOG.md` 检查是否有相关待办
- **大阶段检查点时回顾**：每个大阶段完成后，扫一遍 BACKLOG 看有没有顺手能解决的
- **修复时同步更新状态**：开始修复某条目时注明进行中，完成后按上述流程关闭
- **修复失败/搁置时**：保留在原严重等级区域，追加说明失败原因或搁置理由

#### 禁止行为

- **禁止不验证就关闭**：改了代码不跑测试，直接标已完成
- **禁止遗忘更新状态**：修复了某个 BACKLOG 条目但不更新 `BACKLOG.md`，导致条目状态与实际脱节
- **禁止批量静默关闭**：不能一次性把多个条目标为已完成而不逐条说明验证方式
- **禁止删除未解决条目**：还没修的条目不允许删除，只能保留在对应严重等级区域中
- **禁止打乱排序**：新增条目必须插入对应等级末尾，不得随意插在其他等级中间
