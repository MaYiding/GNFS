...
CLAUDE.md 只承载**全局操作规范 + 关键约定 / 铁律**。凡"特别具体的设计细节"——单个 `GNFS_*` 开关 / helper 的算法、ENV 解析规则、bit-for-bit 证明、ROI 分析、集成点、测试清单——一律写入 [`docs/`](docs/),CLAUDE.md 仅留一句话速查 + 链接。

### 判据:写 CLAUDE.md 还是写 docs?

- **写某段代码 / 跑某个实验时才查**的具体细节 → `docs/`
- **每次开发都要遵守**的流程 / 约定 / 铁律 → CLAUDE.md

### 新增 `GNFS_*` 开关 / helper 时

1. **详细文档写到 [`docs/env-flags/<module>.md`](docs/env-flags/README.md)**(对应代码模块,追加一节),**不写进 CLAUDE.md**。
